...
app = FastAPI(title="World Server AI3D Worker", version="1.1.0")
...
async def _stream_upload(upload: UploadFile, destination: Path, max_bytes: int, kind: str) -> int:
    ...
    with destination.open("wb") as handle:
        while True:
            ...
            size += len(chunk)
            if size > max_bytes:
                raise HTTPException(status_code=413, detail=f"{kind} exceeds {max_bytes // (1024 * 1024)} MB limit.")
            handle.write(chunk)
    return size
...
async def create_job(
    ...
    supported = {"auto", "image_to_3d", "depth", "building", "map", "voxel_city", "texture_optimize"}
    ...
    if (needs_image or needs_texture) and file is None:
