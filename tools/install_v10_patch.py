...
def main():
    ...
    if backup.exists():
        ...
        try:
            ...
            new['history']=(old.get('history',[])+new.get('history',[]))[-500:]; (nk/'quality-ratchet.json').write_text(json.dumps(new,indent=2)+'\n')
        except Exception: pass
    cmds=[[sys.executable,'tools/migrate_worlds_to_v10.py'],[sys.executable,'tools/prepare_world_v10.py'],[sys.executable,'tools/auto_repair.py','--apply'],[sys.executable,'tools/self_heal_protected_errors.py','--apply'],[sys.executable,'tools/error_immunity.py','compile'],[sys.executable,'tools/compile_protection_pack.py'],[sys.executable,'tools/quality_pipeline.py'],[sys.executable,'tools/quality_rollout.py','--repo',str(target),'--mode','promote'],[sys.executable,'tools/verify_consumers.py','--repo',str(target),'--scope','all'],[sys.executable,'tools/consumer_drift_audit.py','--repo',str(target)],[sys.executable,'tools/readiness.py']]
    results=[]
    for cmd in cmds:
        p=subprocess.run(cmd,cwd=dest,text=True,capture_output=True); results.append({'cmd':' '.join(cmd),'pass':p.returncode==0,'stdout':p.stdout[-3000:],'stderr':p.stderr[-2000:]})
        if p.returncode:
