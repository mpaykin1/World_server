...
const FACE_UV_BY_MATERIAL=Array.from({length:14},(_,materialId)=>{const tile=Math.max(0,materialId-1),tx=tile%VOXEL_ATLAS_COLS,ty=Math.floor(tile/VOXEL_ATLAS_COLS);return FACE_UV.map(q=>[(tx+q[0])/VOXEL_ATLAS_COLS,1-(ty+q[1])/VOXEL_ATLAS_COLS]);});
function pushFace(arr,x,y,z,face,color,vertexShade=null,shore=0,materialId=0){const base=arr.pos.length/3,col=faceColorScratch.set(color),faceUv=FACE_UV_BY_MATERIAL[materialId];for(let i=0;i<face.v.length;i++){const v=face.v[i],shade=face.shade*(vertexShade?.[i]??1);arr.pos.push(x+v[0],y+v[1],z+v[2]);if(arr.nor)arr.nor.push(face.d[0],face.d[1],face.d[2]);arr.col.push(Math.round(Math.max(0,Math.min(1,col.r*shade))*255),Math.round(Math.max(0,Math.min(1,col.g*shade))*255),Math.round(Math.max(0,Math.min(1,col.b*shade))*255));if(arr.shore)arr.shore.push(shore);if(arr.mat)arr.mat.push(materialId);if(arr.uv){const q=faceUv[i];arr.uv.push(q[0],q[1]);}}arr.idx.push(base,base+1,base+2,base,base+2,base+3);}
function makeGeometry(data){ const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.BufferAttribute(new Int16Array(data.pos),3)); g.setAttribute('color',new THREE.BufferAttribute(new Uint8Array(data.col),3,true)); if(data.nor?.length)g.setAttribute('normal',new THREE.BufferAttribute(new Int8Array(data.nor),3)); if(data.uv?.length)g.setAttribute('uv',new THREE.BufferAttribute(Uint16Array.from(data.uv,v=>Math.round(v*65535)),2,true)); if(data.shore?.length)g.setAttribute('goldenShore',new THREE.BufferAttribute(Uint16Array.from(data.shore,v=>Math.round(v*65535)),1,true)); if(data.mat?.length)g.setAttribute('goldenMaterial',new THREE.BufferAttribute(new Uint8Array(data.mat),1)); g.setIndex(data.idx?.length?new THREE.BufferAttribute(data.pos.length/3<=65535?new Uint16Array(data.idx):new Uint32Array(data.idx),1):data.idx); if(!data.nor?.length)g.computeVertexNormals(); g.boundingSphere=new THREE.Sphere(new THREE.Vector3(CHUNK/2,WORLD_Y/2,CHUNK/2),Math.hypot(CHUNK/2,WORLD_Y/2,CHUNK/2)); return g; }
