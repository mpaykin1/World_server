...
function makeGeometry(data){ const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.Float32BufferAttribute(data.pos,3)); g.setAttribute('color',new THREE.Float32BufferAttribute(data.col,3)); if(data.nor?.length)g.setAttribute('normal',new THREE.Float32BufferAttribute(data.nor,3)); if(data.uv?.length)g.setAttribute('uv',new THREE.Float32BufferAttribute(data.uv,2)); if(data.shore?.length)g.setAttribute('goldenShore',new THREE.Float32BufferAttribute(data.shore,1)); if(data.mat?.length)g.setAttribute('goldenMaterial',new THREE.Float32BufferAttribute(data.mat,1)); g.setIndex(data.idx); if(!data.nor?.length)g.computeVertexNormals(); g.computeBoundingSphere(); return g; }
