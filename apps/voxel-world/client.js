...
function makeGeometry(data){ const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.Float32BufferAttribute(data.pos,3)); g.setAttribute('color',new THREE.Float32BufferAttribute(data.col,3)); if(data.nor?.length)g.setAttribute('normal',new THREE.BufferAttribute(new Int8Array(data.nor),3)); if(data.uv?.length)g.setAttribute('uv',new THREE.Float32BufferAttribute(data.uv,2)); if(data.shore?.length)g.setAttribute('goldenShore',new THREE.BufferAttribute(Uint16Array.from(data.shore,v=>Math.round(v*65535)),1,true)); if(data.mat?.length)g.setAttribute('goldenMaterial',new THREE.BufferAttribute(new Uint8Array(data.mat),1)); g.setIndex(data.idx?.length?new THREE.BufferAttribute(data.pos.length/3<=65535?new Uint16Array(data.idx):new Uint32Array(data.idx),1):data.idx); if(!data.nor?.length)g.computeVertexNormals(); g.boundingSphere=new THREE.Sphere(new THREE.Vector3(CHUNK/2,WORLD_Y/2,CHUNK/2),Math.hypot(CHUNK/2,WORLD_Y/2,CHUNK/2)); return g; }
