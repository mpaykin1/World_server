...
function pushFace(arr,x,y,z,face,vertexShade=null,shore=0,materialId=0){const faceColors=FACE_COLOR_BY_MATERIAL[materialId][face.colorIndex],faceUv=FACE_UV_BY_MATERIAL[materialId],packedShore=arr.shore?Math.round(shore*65535):0,isSolid=arr.mat!==undefined;for(let i=0;i<face.v.length;i++){const v=face.v[i],packedColor=faceColors[vertexShade?.[i]??0];arr.pos.push(x+v[0],y+v[1],z+v[2]);arr.nor.push(face.d[0],face.d[1],face.d[2]);arr.col.push(packedColor[0],packedColor[1],packedColor[2]);if(isSolid){arr.mat.push(materialId);const q=faceUv[i];arr.uv.push(q[0],q[1]);}else if(arr.shore)arr.shore.push(packedShore);}}
