...
function pushFace(arr,x,y,z,face,vertexShade=null,shore=0,materialId=0){const faceColors=FACE_COLOR_BY_MATERIAL[materialId][face.colorIndex],faceUv=FACE_UV_BY_MATERIAL[materialId],packedShore=arr.shore?Math.round(shore*65535):0,isSolid=arr.mat!==undefined,v0=face.v[0],v1=face.v[1],v2=face.v[2],v3=face.v[3],nx=face.d[0],ny=face.d[1],nz=face.d[2];arr.pos.push(x+v0[0],y+v0[1],z+v0[2],x+v1[0],y+v1[1],z+v1[2],x+v2[0],y+v2[1],z+v2[2],x+v3[0],y+v3[1],z+v3[2]);arr.nor.push(nx,ny,nz,nx,ny,nz,nx,ny,nz,nx,ny,nz);const c0=faceColors[vertexShade?.[0]??0],c1=faceColors[vertexShade?.[1]??0],c2=faceColors[vertexShade?.[2]??0],c3=faceColors[vertexShade?.[3]??0];arr.col.push(c0[0],c0[1],c0[2],c1[0],c1[1],c1[2],c2[0],c2[1],c2[2],c3[0],c3[1],c3[2]);if(isSolid){arr.mat.push(materialId,materialId,materialId,materialId);arr.uv.push(faceUv[0][0],faceUv[0][1],faceUv[1][0],faceUv[1][1],faceUv[2][0],faceUv[2][1],faceUv[3][0],faceUv[3][1]);}else if(arr.shore)arr.shore.push(packedShore,packedShore,packedShore,packedShore);}
function makeGeometry(data){ const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.BufferAttribute(Int16Array.from(data.pos),3)); g.setAttribute('color',new THREE.BufferAttribute(Uint8Array.from(data.col),3,true)); if(data.nor?.length)g.setAttribute('normal',new THREE.BufferAttribute(Int8Array.from(data.nor),3)); if(data.uv?.length)g.setAttribute('uv',new THREE.BufferAttribute(Uint16Array.from(data.uv),2,true)); if(data.shore?.length)g.setAttribute('goldenShore',new THREE.BufferAttribute(Uint16Array.from(data.shore),1,true)); if(data.mat?.length)g.setAttribute('goldenMaterial',new THREE.BufferAttribute(Uint8Array.from(data.mat),1)); const vertexCount=data.pos.length/3,quadCount=vertexCount/4,IndexArray=vertexCount<=65535?Uint16Array:Uint32Array,indices=new IndexArray(quadCount*6); for(let q=0,i=0;q<quadCount;q++){const base=q*4;indices[i++]=base;indices[i++]=base+1;indices[i++]=base+2;indices[i++]=base;indices[i++]=base+2;indices[i++]=base+3;} g.setIndex(new THREE.BufferAttribute(indices,1)); if(!data.nor?.length)g.computeVertexNormals(); g.boundingSphere=new THREE.Sphere(new THREE.Vector3(CHUNK/2,WORLD_Y/2,CHUNK/2),Math.hypot(CHUNK/2,WORLD_Y/2,CHUNK/2)); return g; }
...
async function rebuildChunkIncremental(c){
  ...
  for(let lx=0;lx<CHUNK;lx++){for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx];for(let y=0;y<=top;y++){
    const b=localBlocks[(y*CHUNK+lz)*CHUNK+lx];if(b===BLOCK.AIR)continue;const isWater=b===BLOCK.WATER,isTranslucent=TRANSLUCENT_BY_BLOCK[b]===true,dst=isWater?water:(isTranslucent?translucent:solid);
    for(const f of FACE){const nb=localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]);const visible=FACE_VISIBLE_BY_BLOCK[b][nb];if(!visible)continue;const ao=isWater?null:faceCornerAO(lx,y,lz,f,localBlock),shore=(isWater&&f.d[1]===1)?waterShoreAt(lx,y,lz,localBlock):0;pushFace(dst,lx,y,lz,f,ao,shore,b);}
  }}if((lx+1)%columnsPerSlice===0&&lx+1<CHUNK)await yieldChunkBuild();}
  for(const [data,mat] of [[solid,solidMaterial],[translucent,transparentMaterial],[water,waterMaterial]])if(data.pos.length){const m=new THREE.Mesh(makeGeometry(data),mat);m.position.set(bx,0,bz);m.receiveShadow=true;m.castShadow=mat===solidMaterial;c.meshes.push(m);worldGroup.add(m);}
}
