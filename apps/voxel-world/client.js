...
function pushFace(arr,x,y,z,face,color,vertexShade=null,shore=0,materialId=0){const base=arr.pos.length/3,col=faceColorScratch.set(color),tile=Math.max(0,materialId-1),tx=tile%VOXEL_ATLAS_COLS,ty=Math.floor(tile/VOXEL_ATLAS_COLS);for(let i=0;i<face.v.length;i++){const v=face.v[i],shade=face.shade*(vertexShade?.[i]??1);arr.pos.push(x+v[0],y+v[1],z+v[2]);if(arr.nor)arr.nor.push(face.d[0],face.d[1],face.d[2]);arr.col.push(col.r*shade,col.g*shade,col.b*shade);if(arr.shore)arr.shore.push(shore);if(arr.mat)arr.mat.push(materialId);if(arr.uv){const q=FACE_UV[i];arr.uv.push((tx+q[0])/VOXEL_ATLAS_COLS,1-(ty+q[1])/VOXEL_ATLAS_COLS);}}arr.idx.push(base,base+1,base+2,base,base+2,base+3);}
