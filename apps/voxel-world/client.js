...
function rebuildChunk(c){
  ...
  for(let lx=0;lx<CHUNK;lx++)for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx];for(let y=0;y<=top;y++){
    ...
    for(const f of FACE){ const nb=localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]); let visible=false;
      if(isWater) visible=nb!==BLOCK.WATER&&nb===BLOCK.AIR;`r`n      else if(isTranslucent) visible=nb===BLOCK.AIR||nb===BLOCK.WATER;
...
async function rebuildChunkIncremental(c){
  ...
  for(let lx=0;lx<CHUNK;lx++){for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx];for(let y=0;y<=top;y++){
    const b=c.get(lx,y,lz);if(b===BLOCK.AIR)continue;const isWater=b===BLOCK.WATER,isTranslucent=!isWater&&BLOCKS[b]?.alpha!==undefined,dst=isWater?water:(isTranslucent?translucent:solid),blockColor=BLOCKS[b].color;`r`n    for(const f of FACE){const nb=localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]);let visible=false;if(isWater)visible=nb!==BLOCK.WATER&&nb===BLOCK.AIR;else if(isTranslucent)visible=nb===BLOCK.AIR||nb===BLOCK.WATER;else visible=!isOccluding(nb)||BLOCKS[nb]?.alpha!==undefined;if(!visible)continue;const ao=isWater?null:faceCornerAO(lx,y,lz,f,localBlock),shore=(isWater&&f.d[1]===1)?waterShoreAt(lx,y,lz,localBlock):0;pushFace(dst,lx,y,lz,f,blockColor,ao,shore,b);}
  }}if((lx+1)%columnsPerSlice===0&&lx+1<CHUNK)await yieldChunkBuild();}
  for(const [data,mat] of [[solid,solidMaterial],[translucent,transparentMaterial],[water,waterMaterial]])if(data.idx.length){const m=new THREE.Mesh(makeGeometry(data),mat);m.position.set(bx,0,bz);m.receiveShadow=true;m.castShadow=mat===solidMaterial;c.meshes.push(m);worldGroup.add(m);}
}
