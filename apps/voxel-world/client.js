...
function faceCornerAO(lx,y,lz,face,getBlock){
  const n=face.d,a0=n[0]?1:0,a1=n[0]?2:(n[1]?2:1),out=FACE_AO_SCRATCH,sideA=FACE_AO_SIDE_A,sideB=FACE_AO_SIDE_B;
  for(let si=0;si<2;si++){const step=si?1:-1;let ax=n[0],ay=n[1],az=n[2],bx=ax,by=ay,bz=az;if(a0===0)ax+=step;else if(a0===1)ay+=step;else az+=step;if(a1===0)bx+=step;else if(a1===1)by+=step;else bz+=step;sideA[si]=isOccluding(getBlock(lx+ax,y+ay,lz+az))?1:0;sideB[si]=isOccluding(getBlock(lx+bx,y+by,lz+bz))?1:0;}
  for(let i=0;i<4;i++){const v=face.v[i],s1=v[a0]?1:-1,s2=v[a1]?1:-1,o1=sideA[s1>0?1:0],o2=sideB[s2>0?1:0];let cx=n[0],cy=n[1],cz=n[2];if(a0===0)cx+=s1;else if(a0===1)cy+=s1;else cz+=s1;if(a1===0)cx+=s2;else if(a1===1)cy+=s2;else cz+=s2;const oc=o1&&o2?0:(isOccluding(getBlock(lx+cx,y+cy,lz+cz))?1:0);out[i]=o1&&o2?3:o1+o2+oc;}
  return out;
}
...
function pushFace(arr,x,y,z,face,vertexShade=null,shore=0,materialId=0){const base=arr.pos.length/3,faceColors=FACE_COLOR_BY_MATERIAL[materialId][face.colorIndex],faceUv=FACE_UV_BY_MATERIAL[materialId],packedShore=arr.shore?Math.round(shore*65535):0,isSolid=arr.mat!==undefined;for(let i=0;i<face.v.length;i++){const v=face.v[i],packedColor=faceColors[vertexShade?.[i]??0];arr.pos.push(x+v[0],y+v[1],z+v[2]);arr.nor.push(face.d[0],face.d[1],face.d[2]);arr.col.push(packedColor[0],packedColor[1],packedColor[2]);if(isSolid){arr.mat.push(materialId);const q=faceUv[i];arr.uv.push(q[0],q[1]);}else if(arr.shore)arr.shore.push(packedShore);}arr.idx.push(base,base+1,base+2,base,base+2,base+3);}
...
function rebuildChunk(c){
  ...
  for(let lx=0;lx<CHUNK;lx++)for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx];for(let y=0;y<=top;y++){
    const b=c.get(lx,y,lz); if(b===BLOCK.AIR) continue; const isWater=b===BLOCK.WATER,isTranslucent=TRANSLUCENT_BY_BLOCK[b]===true,dst=isWater?water:(isTranslucent?translucent:solid);
    for(const f of FACE){ const nb=localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]); let visible=false;
      ...
      else visible=nb===BLOCK.AIR||nb===BLOCK.WATER||TRANSLUCENT_BY_BLOCK[nb]===true;
...
async function rebuildChunkIncremental(c){
  ...
  for(let lx=0;lx<CHUNK;lx++){for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx];for(let y=0;y<=top;y++){
    const b=c.get(lx,y,lz);if(b===BLOCK.AIR)continue;const isWater=b===BLOCK.WATER,isTranslucent=TRANSLUCENT_BY_BLOCK[b]===true,dst=isWater?water:(isTranslucent?translucent:solid);
    for(const f of FACE){const nb=localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]);let visible=false;if(isWater)visible=nb===BLOCK.AIR;else if(isTranslucent)visible=nb===BLOCK.AIR||nb===BLOCK.WATER;else visible=nb===BLOCK.AIR||nb===BLOCK.WATER||TRANSLUCENT_BY_BLOCK[nb]===true;if(!visible)continue;const ao=isWater?null:faceCornerAO(lx,y,lz,f,localBlock),shore=(isWater&&f.d[1]===1)?waterShoreAt(lx,y,lz,localBlock):0;pushFace(dst,lx,y,lz,f,ao,shore,b);}
  }}if((lx+1)%columnsPerSlice===0&&lx+1<CHUNK)await yieldChunkBuild();}
  for(const [data,mat] of [[solid,solidMaterial],[translucent,transparentMaterial],[water,waterMaterial]])if(data.idx.length){const m=new THREE.Mesh(makeGeometry(data),mat);m.position.set(bx,0,bz);m.receiveShadow=true;m.castShadow=mat===solidMaterial;c.meshes.push(m);worldGroup.add(m);}
}
