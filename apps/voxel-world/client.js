...
function makeGeometry(data){ const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.BufferAttribute(new Int16Array(data.pos),3)); g.setAttribute('color',new THREE.BufferAttribute(new Uint8Array(data.col),3,true)); if(data.nor?.length)g.setAttribute('normal',new THREE.BufferAttribute(new Int8Array(data.nor),3)); if(data.uv?.length)g.setAttribute('uv',new THREE.BufferAttribute(new Uint16Array(data.uv),2,true)); if(data.shore?.length)g.setAttribute('goldenShore',new THREE.BufferAttribute(new Uint16Array(data.shore),1,true)); if(data.mat?.length)g.setAttribute('goldenMaterial',new THREE.BufferAttribute(new Uint8Array(data.mat),1)); const vertexCount=data.pos.length/3,quadCount=vertexCount/4,IndexArray=vertexCount<=65535?Uint16Array:Uint32Array,indices=new IndexArray(quadCount*6); for(let q=0,i=0;q<quadCount;q++){const base=q*4;indices[i++]=base;indices[i++]=base+1;indices[i++]=base+2;indices[i++]=base;indices[i++]=base+2;indices[i++]=base+3;} g.setIndex(new THREE.BufferAttribute(indices,1)); if(!data.nor?.length)g.computeVertexNormals(); g.boundingSphere=new THREE.Sphere(new THREE.Vector3(CHUNK/2,WORLD_Y/2,CHUNK/2),Math.hypot(CHUNK/2,WORLD_Y/2,CHUNK/2)); return g; }
