...
const FACE_UV_PACKED_BY_MATERIAL=FACE_UV_BY_MATERIAL.map(uv=>[uv[0][0],uv[0][1],uv[1][0],uv[1][1],uv[2][0],uv[2][1],uv[3][0],uv[3][1]]);
function pushFace(arr,x,y,z,face,vertexShade=null,shore=0,materialId=0){const isSolid=arr.mat!==undefined,v=face.posOffset,nx=face.d[0],ny=face.d[1],nz=face.d[2];arr.pos.push(x+v[0],y+v[1],z+v[2],x+v[3],y+v[4],z+v[5],x+v[6],y+v[7],z+v[8],x+v[9],y+v[10],z+v[11]);arr.nor.push(nx,ny,nz,nx,ny,nz,nx,ny,nz,nx,ny,nz);if(arr.col){const faceColors=FACE_COLOR_BY_MATERIAL[materialId][face.colorIndex],c0=faceColors[vertexShade[0]],c1=faceColors[vertexShade[1]],c2=faceColors[vertexShade[2]],c3=faceColors[vertexShade[3]];arr.col.push(c0[0],c0[1],c0[2],c1[0],c1[1],c1[2],c2[0],c2[1],c2[2],c3[0],c3[1],c3[2]);}if(isSolid){const faceUv=FACE_UV_PACKED_BY_MATERIAL[materialId];arr.mat.push(materialId,materialId,materialId,materialId);arr.uv.push(faceUv[0],faceUv[1],faceUv[2],faceUv[3],faceUv[4],faceUv[5],faceUv[6],faceUv[7]);}else if(arr.shore){const packedShore=Math.round(shore*65535);arr.shore.push(packedShore,packedShore,packedShore,packedShore);}}
...
function quadIndexU32(quadCount){const needed=quadCount*6;if(QUAD_INDEX_U32.length<needed){const out=new Uint32Array(needed);for(let q=0,i=0;q<quadCount;q++){const base=q*4;out[i++]=base;out[i++]=base+1;out[i++]=base+2;out[i++]=base;out[i++]=base+2;out[i++]=base+3;}QUAD_INDEX_U32=out;}return QUAD_INDEX_U32.subarray(0,needed);}
function makeGeometry(data){ const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.BufferAttribute(Int16Array.from(data.pos),3)); if(data.col)g.setAttribute('color',new THREE.BufferAttribute(Uint8Array.from(data.col),3,true)); if(data.nor?.length)g.setAttribute('normal',new THREE.BufferAttribute(Int8Array.from(data.nor),3)); if(data.uv?.length)g.setAttribute('uv',new THREE.BufferAttribute(Uint16Array.from(data.uv),2,true)); if(data.shore?.length)g.setAttribute('goldenShore',new THREE.BufferAttribute(Uint16Array.from(data.shore),1,true)); if(data.mat?.length)g.setAttribute('goldenMaterial',new THREE.BufferAttribute(Uint8Array.from(data.mat),1)); const vertexCount=data.pos.length/3,quadCount=vertexCount/4,useU16=vertexCount<=65535; g.setIndex(useU16?QUAD_INDEX_U16_ATTRIBUTE:new THREE.BufferAttribute(quadIndexU32(quadCount),1)); if(useU16)g.setDrawRange(0,quadCount*6); if(!data.nor?.length)g.computeVertexNormals(); g.boundingSphere=new THREE.Sphere(new THREE.Vector3(CHUNK/2,WORLD_Y/2,CHUNK/2),Math.hypot(CHUNK/2,WORLD_Y/2,CHUNK/2)); return g; }
function rebuildChunk(c){
  ...
  for(let lx=0;lx<CHUNK;lx++)for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx],columnInterior=lx>0&&lx<CHUNK-1&&lz>0&&lz<CHUNK-1;for(let y=0;y<=top;y++){
    ...
    for(let fi=0;fi<6;fi++){ const f=FACE[fi],nb=interior?localBlocks[blockIndex+f.blockOffset]:localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]); const visible=faceVisible[nb];
      if(!visible) continue; const ao=isWater?null:(interior?faceCornerAOInterior(blockIndex,f,localBlocks):faceCornerAO(lx,y,lz,f,localBlock)); const shore=(isWater&&f.d[1]===1)?waterShoreAt(lx,y,lz,localBlock):0; pushFace(dst,lx,y,lz,f,ao,shore,b);
    }
  }}
...
async function rebuildChunkIncremental(c){
  ...
  for(let lx=0;lx<CHUNK;lx++){for(let lz=0;lz<CHUNK;lz++){const top=c.columnTop[lz*CHUNK+lx],columnInterior=lx>0&&lx<CHUNK-1&&lz>0&&lz<CHUNK-1;for(let y=0;y<=top;y++){
    const blockIndex=(y*CHUNK+lz)*CHUNK+lx,b=localBlocks[blockIndex];if(b===BLOCK.AIR)continue;const isWater=b===BLOCK.WATER,isTranslucent=TRANSLUCENT_BY_BLOCK[b]===true,dst=isWater?water:(isTranslucent?translucent:solid),faceVisible=FACE_VISIBLE_BY_BLOCK[b];
    const interior=columnInterior&&y>0&&y<WORLD_Y-1;for(let fi=0;fi<6;fi++){const f=FACE[fi],nb=interior?localBlocks[blockIndex+f.blockOffset]:localBlock(lx+f.d[0],y+f.d[1],lz+f.d[2]);const visible=faceVisible[nb];if(!visible)continue;const ao=isWater?null:(interior?faceCornerAOInterior(blockIndex,f,localBlocks):faceCornerAO(lx,y,lz,f,localBlock)),shore=(isWater&&f.d[1]===1)?waterShoreAt(lx,y,lz,localBlock):0;pushFace(dst,lx,y,lz,f,ao,shore,b);}
  }}if((lx+1)%columnsPerSlice===0&&lx+1<CHUNK)await yieldChunkBuild();}
  for(const [data,mat] of [[solid,solidMaterial],[translucent,transparentMaterial],[water,waterMaterial]])if(data.pos.length){const m=new THREE.Mesh(makeGeometry(data),mat);m.position.set(bx,0,bz);m.receiveShadow=true;m.castShadow=mat===solidMaterial;c.meshes.push(m);worldGroup.add(m);}
}
