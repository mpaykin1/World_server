...
replayButton.addEventListener('click',()=>{replay();}); addEventListener('keydown',e=>{if(e.code==='Space'){e.preventDefault();replay();}});
...
const player={x:9.3,z:9.2,yaw:-2.38,pitch:-.08,height:1.64,radius:.34};
...
wirePad('movePad','moveKnob',touchMove);wirePad('lookPad','lookKnob',touchLook);
...
window.__CINEMATIC_ENCOUNTER_READY__={ready:true,version:'1.0.0',features:['procedural-silhouette','dust-fog','muzzle-flash','bloom','light-cone','sparks','camera-shake','adaptive-dpr','desktop-controls','mobile-dual-joystick','collision','step-up'],snapshot:()=>({player:[player.x,groundHeightAt(player.x,player.z),player.z],yaw:player.yaw,pitch:player.pitch,calls:renderer.info.render.calls,dpr})};
