...
(function () {
  ...
  // GOLDEN_MOBILE_INPUT_HARDENING_V3 — one canonical fix, reused by every mobile game.
  // maxTouchPoints is NOT required for a UA-confirmed mobile browser: some
  // WebKit builds (notably Playwright's mobile-webkit/iPhone device
  // emulation) report maxTouchPoints=0 despite a real iPhone UA and a
  // coarse pointer, which previously made this whole check return false
  // and silently dropped the joystick - see
  // data/mobile-playable-ui-known-failures-v3.json for the regression
  // this caused. maxTouchPoints is still required for the desktop-
  // touchscreen fallback branch, since THAT branch is the one guarding
  // against false positives with no UA signal to trust instead.
  function needsTouchControls() {
    const mm = q => { try { return matchMedia(q).matches === true; } catch { return false; } };
    const touch = (navigator.maxTouchPoints || 0) > 0;
    const ua = navigator.userAgent || '';
    const mobileUA = /Android|iPhone|iPad|iPod|Mobile/i.test(ua) || (/Macintosh/i.test(ua) && touch && (navigator.maxTouchPoints || 0) > 1);
    if (mobileUA) return true;
    const compact = Math.min(screen.width || innerWidth, screen.height || innerHeight) <= 1180;
    return touch && (mm('(pointer:coarse)') || mm('(any-pointer:coarse)')) && compact && !/Windows NT|X11/i.test(ua);
  }
  ...
  function reportReady(details={}) {
    ...
    state.ready =
      ...
      (!needsTouchControls() || state.touchControls === true);
  ...
  function installMobileControls() {
    ...
    const existingGolden = document.getElementById('goldenMobileControls');
    if (existingGolden && existingGolden.querySelector('#goldenMovePad') && existingGolden.querySelector('#goldenLookZone')) {
      state.touchControls = true;
      state.mobileReady = true;
      return;
    }
    if (existingGolden) existingGolden.remove(); // malformed canonical root: repair instead of false-green
