...
- **Параллельный AI — обязательная проверка перед push:** перед каждым `commit/push` выполнять `git fetch --prune`, сравнивать `HEAD` vs `origin/<branch>`, проверять `git log --oneline` на наличие чужих пушей. Не перезаписывать remote, никогда `git push --force`. Если remote ушёл вперёд или ветка общая — сохранять свою работу в отдельную `checkpoint/desktop-ai-<timestamp>` ветку (`git checkout -b checkpoint/desktop-ai-$(date +%Y%m%d-%H%M%S) && git push -u origin checkpoint/...`), не делать `rebase/reset` чужой незавершённой работы.
- **Dummy-тесты — временно:** `test/*dummy*` для `legacy-evidence-factory` не заменяют реальные `regression/integration` проверки; каждый dummy должен быть постепенно заменён на настоящий тест, который проверяет реальную capability (существование файла + выполнение команды + проверка результата).
- **Метрики — честно:** `nativeAstAdapterAvailable:true` выставлять только если `typescript`/`sg` реально установлен и `NATIVE_AST_DATAFLOW_REPORT.json` `nativeAstAvailable:true`; иначе `false` + `fallback-v5` — это валидный PASS для `lexical` режима, контракт должен ожидать `false`, а не искусственное `true`. Аналогично `GRAPHICS` — только реальные baselines/comparison, не синтетический `QUALITY_CHANGE_EVIDENCE.json`.
