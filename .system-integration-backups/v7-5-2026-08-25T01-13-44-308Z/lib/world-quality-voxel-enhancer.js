...
const {detailBudget}=require('./world-quality-detail-budget');
...
function enhanceVoxelWorld(world,options={}){
  ...
  for(const c of cells.values()){
    ...
    if(cornice)stats.corniceCandidates++;if(pillar)stats.pillarCandidates++;const importance=clamp(score/2.25,0,1),hero=!!intent.spireLike||(roof&&c.y>semantic.yMax*.72),budget=detailBudget({importance,hero,pressure:clamp(+options.pressure||0,0,1),mobile:!!options.mobile});candidates.push({c,intent,edge,roof,cornice,pillar,score:score*(.65+budget.voxelScale*.35),budget});
  }
  ...
  const enhanced=source.concat(additions);if(!sameFrontProjection(source,enhanced))throw new Error('WORLD_QUALITY_AUTOPILOT_V5 front-projection invariant violated');
