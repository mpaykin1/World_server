...
create index if not exists gap_closure_registry_status_idx
  on public.gap_closure_registry(status, severity, last_seen_at desc);
create index if not exists gap_closure_registry_domain_idx
  on public.gap_closure_registry(domain, status);
...
create index if not exists gap_closure_evidence_gap_idx
  on public.gap_closure_evidence(gap_key, observed_at desc);
...
begin
  select status='closed' into v_was_closed
  from public.gap_closure_registry where gap_key=p_gap_key;
  ...
  if p_present then
    ...
    on conflict (gap_key) do update set
      ...
      status=case
        when public.gap_closure_registry.status='closed' then excluded.status
        else public.gap_closure_registry.status
      end,
...
revoke all on function private.gap_closure_sync_gap(
  text,text,text,text,text,boolean,boolean,jsonb,jsonb,timestamptz
...
begin
  ...
  v_change := private.gap_closure_sync_gap(
    ...
    'At least one fresh runtime evidence channel is required before runtime quality can be verified.',
    ...
    jsonb_build_object('action','refresh-runtime-probe','closeOnlyAfter','fresh accepted runtime evidence'),v_now);
  if v_change=1 then v_detected:=v_detected+1;
  elsif v_change=2 then v_detected:=v_detected+1; v_reopened:=v_reopened+1;
  elsif v_change=-1 then v_closed:=v_closed+1; end if;
  ...
  v_change := private.gap_closure_sync_gap(
    ...
    'Production readiness must include verified iOS and Android evidence, not emulation only.',
  ...
  v_change := private.gap_closure_sync_gap(
    'supabase.security.security-definer-exposure','security','major',
    'SECURITY DEFINER functions have exposed EXECUTE grants',
    'Privileged database functions callable by anon/authenticated roles require explicit least-privilege review.',
  ...
  if v_latest_runtime_evidence is null or v_latest_runtime_evidence < v_now - interval '20 minutes' then
    ...
    insert into public.quality_worker_jobs(
      job_key,kind,priority,required_capabilities,payload,status,attempts,max_attempts,created_at,updated_at
    )
    select v_job_key,'runtime.telemetry.refresh',100,'[]'::jsonb,
      jsonb_build_object('gapKey','runtime.telemetry.stale'),'queued',0,3,v_now,v_now
  ...
  if v_profiles > 0 and v_manifests = 0 then
    ...
    select v_job_key,'pixel.atlas.build',80,'[]'::jsonb,
      jsonb_build_object('gapKey','pixel.animation.atlas.missing'),'queued',0,3,v_now,v_now
    where not exists(select 1 from public.quality_worker_jobs where job_key=v_job_key);
    if found then v_queued := v_queued + 1; end if;
  end if;
  ...
  return jsonb_build_object(
    'ok',true,'runId',v_run_id,'detected',v_detected,'reopened',v_reopened,'closed',v_closed,'queued',v_queued
  );
end;
$$;
...
begin
  ...
  perform cron.schedule(
    'gap-closure-db-v1',
    '*/5 * * * *',
    $cmd$select public.run_gap_closure_db_cycle('cron');$cmd$
  );
end $$;
