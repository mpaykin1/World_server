alter table public.game_player_states
  add column if not exists last_position_at timestamptz not null default now();
...
begin
  ...
  if not isfinite(requested_x) or not isfinite(requested_z) or abs(requested_x) > 10000 or abs(requested_z) > 10000 then
