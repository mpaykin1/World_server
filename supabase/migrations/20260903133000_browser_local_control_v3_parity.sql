...
-- Preserve LIVE security model: SECURITY DEFINER, private.browser_ai_worker_tokens with worker/active/expires_at/token_hash
...
-- Drop old token heartbeat overloads to recreate with p_version and correct SECURITY DEFINER
...
declare
  ...
  v_valid boolean;
begin
  -- LIVE-compatible worker-specific token validation: checks private.browser_ai_worker_tokens
  -- Do NOT use private.browser_ai_validate_worker_token (does not exist LIVE)
  select exists(
...
-- Backward compat: 5-arg overload forwards to 6-arg with default version (for workers not yet updated)
create or replace function public.browser_ai_worker_heartbeat(
  p_worker text,
  p_token text,
