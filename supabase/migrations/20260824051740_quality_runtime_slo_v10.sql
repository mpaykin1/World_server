...
begin
  v_run_key:='runtime-v10-'||to_char(v_now,'YYYYMMDDHH24MI');
  ...
  v_probe_at:=nullif(v_probe->>'createdAt','')::timestamptz;
  ...
  if v_probe_bad then v_job:=public.quality_enqueue_worker_job('runtime-synthetic-'||to_char(v_now,'YYYYMMDDHH24'),'runtime.synthetic.failed',100,'["http","repo"]'::jsonb,jsonb_build_object('probe',v_probe,'detectedAt',v_now)); v_enqueued:=v_enqueued||jsonb_build_array(jsonb_build_object('id',v_job,'kind','runtime.synthetic.failed'));
  elsif v_probe_duration>2500 then v_job:=public.quality_enqueue_worker_job('runtime-synthetic-slow-'||to_char(v_now,'YYYYMMDDHH24'),'runtime.synthetic.slow',75,'["http","repo"]'::jsonb,jsonb_build_object('probe',v_probe,'thresholdMs',2500)); v_enqueued:=v_enqueued||jsonb_build_array(jsonb_build_object('id',v_job,'kind','runtime.synthetic.slow')); end if;
  ...
  v_health:=jsonb_build_object('status',v_status,'at',v_now,'telemetry15m',v_count,'errors15m',v_errors,'avgFps15m',v_avg_fps,'p95LoadMs15m',v_p95_load,'lastTelemetryAt',v_last_event,'userTelemetryQuiet',v_user_quiet,'syntheticProbe',v_probe,'syntheticProbeBad',v_probe_bad,'schemaDrift',v_drift,'enqueued',v_enqueued);
  perform public.quality_record_autopilot_run(jsonb_build_object('runKey',v_run_key,'mode','autopilot','status',v_status,'summary',v_health,'evidence',jsonb_build_object('source','pg_cron','version','v10')));
  update private.quality_runtime_state set version='2026-08-24.v10',last_tick_at=v_now,last_health=v_health,last_error=null,consecutive_failures=0,total_ticks=total_ticks+1,updated_at=now() where singleton=true;
  return v_health;
...
select jsonb_build_object('version',s.version,'lastTickAt',s.last_tick_at,'totalTicks',s.total_ticks,'consecutiveFailures',s.consecutive_failures,'lastError',s.last_error,'health',s.last_health,'latestSyntheticProbe',public.quality_latest_synthetic_probe(),'schemaDrift',public.quality_schema_drift_status(),'cronJobs',(select coalesce(jsonb_agg(jsonb_build_object('name',jobname,'schedule',schedule,'active',active) order by jobname),'[]'::jsonb) from cron.job where jobname like 'quality-%')) from private.quality_runtime_state s where singleton=true;
$$;
