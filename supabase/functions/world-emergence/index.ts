...
async function mutateWorld(admin:any,worldId:string,fn:(state:any,seed:number)=>any){
  for(let attempt=0;attempt<3;attempt++){
    ...
    const next=fn(current,seed),now=new Date().toISOString();
...
async function place(admin:any,who:any,b:any){
  ...
  const result=await mutateWorld(admin,worldId,(current,seed)=>{
    const next=normalizeEntity({type,x,z,ownerId:who.id},current.entities.length,seed);
...
async function tick(admin:any,b:any){
  const worldId=safeWorldId(b.worldId);
  const result=await mutateWorld(admin,worldId,(current,seed)=>buildState({entities:current.entities},seed,Math.min(5,current.growthStage+1),current.revision+1));
  return json({worldId,complete:result.emergence.growthStage>=result.emergence.maxGrowthStage,emergence:result.emergence,runtime:"supabase-edge-emergence"});
}
