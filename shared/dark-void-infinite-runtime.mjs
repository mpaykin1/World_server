...
export class RecipeJournal{
 ...
 async #hydrateIndexedDB(){try{const db=await this.#openDB(),stored=await new Promise((resolve,reject)=>{const tx=db.transaction(JSTORE,'readonly'),q=tx.objectStore(JSTORE).get('rows');q.onsuccess=()=>resolve(q.result);q.onerror=()=>reject(q.error)}),local=this.rows,storedValid=verifyJournalRows(stored),localValid=verifyJournalRows(local);if(storedValid&&stored.length&&(!localValid||!local.length||stored.length>=local.length)){this.rows=stored;try{localStorage.setItem(JKEY,JSON.stringify(this.rows.slice(-512)))}catch{}}else if(localValid&&local.length){await this.#putDB(db,local.slice(-512));this.rows=local}else if(storedValid&&stored.length){this.rows=stored;try{localStorage.setItem(JKEY,JSON.stringify(this.rows.slice(-512)))}catch{}}else{this.rows=[];await this.#putDB(db,[]);try{localStorage.setItem(JKEY,'[]')}catch{}}this.persistence='indexedDB';db.close();return true}catch{return false}}
