...
export function verifyJournalRows(rows=[]){if(!Array.isArray(rows))return false;let prev='genesis';for(const r of rows){if(!r||typeof r!=='object')return false;const x={...r};delete x.hash;const h=hash32(prev+'|'+JSON.stringify(x));if(h!==r.hash||r.prevHash!==prev)return false;prev=r.hash}return true}
export class RecipeJournal{
 constructor(){this.rows=this.#load();this.persistence='localStorage';this.recovery={source:'localStorage',restored:this.rows.length,verified:verifyJournalRows(this.rows),hydrated:false,rejectedCorruptSnapshot:false};this.ready=this.#hydrateIndexedDB();this._persistTail=this.ready.catch(()=>false)}
 #load(){try{const rows=JSON.parse(localStorage.getItem(JKEY)||'[]');return verifyJournalRows(rows)?rows:[]}catch{return[]}}
 ...
 async #hydrateIndexedDB(){try{const db=await this.#openDB(),stored=await new Promise((resolve,reject)=>{const tx=db.transaction(JSTORE,'readonly'),q=tx.objectStore(JSTORE).get('rows');q.onsuccess=()=>resolve(q.result);q.onerror=()=>reject(q.error)}),local=this.rows,storedValid=verifyJournalRows(stored),localValid=verifyJournalRows(local),rejectedCorruptSnapshot=Array.isArray(stored)&&stored.length>0&&!storedValid;let source='empty';if(storedValid&&stored.length&&(!localValid||!local.length||stored.length>=local.length)){this.rows=stored;source='indexedDB';try{localStorage.setItem(JKEY,JSON.stringify(this.rows.slice(-512)))}catch{}}else if(localValid&&local.length){await this.#putDB(db,local.slice(-512));this.rows=local;source='localStorage'}else if(storedValid&&stored.length){this.rows=stored;source='indexedDB';try{localStorage.setItem(JKEY,JSON.stringify(this.rows.slice(-512)))}catch{}}else{this.rows=[];await this.#putDB(db,[]);try{localStorage.setItem(JKEY,'[]')}catch{}}this.persistence='indexedDB';this.recovery={source,restored:this.rows.length,verified:verifyJournalRows(this.rows),hydrated:true,rejectedCorruptSnapshot};db.close();return true}catch{this.recovery={...this.recovery,source:'localStorage',restored:this.rows.length,verified:verifyJournalRows(this.rows),hydrated:false,indexedDBUnavailable:true};return false}}
 ...
 recoveryStatus(){return{...this.recovery,persistence:this.persistence,rows:this.rows.length,verified:verifyJournalRows(this.rows)}}
 ...
 verify(){return verifyJournalRows(this.rows)}
