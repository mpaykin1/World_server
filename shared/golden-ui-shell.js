...
(function(){
  ...
  root.innerHTML=`<nav id="goldenToolbar" aria-label="Игровое меню">
    ...
    <div class="goldenTab" data-tab="worlds"><button id="goldenCreateWorld" class="goldenAction">??????? ???</button><div class="goldenWorldMode"><button class="active" data-world-view="newspaper">Газета миров</button><button data-world-view="connections">Связи миров</button></div><div id="goldenWorldList">Загрузка миров…</div><div id="goldenConnections"></div></div>
  ...
  async function createWorld(){const button=root.querySelector('#goldenCreateWorld');if(!window.AppCore?.token?.()){alert('Войдите в аккаунт, чтобы создавать новые миры и менять общий канон.');return;}const idea=prompt('????? ??? ???????? ????????: ?????? ??? ? ??????? ?????? ? ?????');if(!idea?.trim())return;button.disabled=true;button.textContent='?????? ????';try{const requestId=crypto.randomUUID?crypto.randomUUID():worldFactoryGuestId();const payload={action:'create',guestId:worldFactoryGuestId(),requestId,idea:idea.trim()};let result;if(window.AppCore?.api)result=await window.AppCore.api('/api/world-factory',{method:'POST',body:JSON.stringify(payload)});else{const r=await fetch('/api/world-factory',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});result=await r.json();if(!r.ok)throw new Error(result.error||'?? ??????? ??????? ???.');}if(!result?.world?.playUrl)throw new Error('World Factory ?? ?????? ??????? ?????.');location.assign(result.world.playUrl);}catch(error){console.error('[WORLD FACTORY]',error);button.disabled=false;button.textContent='??????? ???';alert(error.message||'?? ??????? ??????? ???.');}}
