...
export function findOriginalWalkingPath({start,goal,cell,maxNodes=300,maxDrop=3,height=2}={}){
 ...
 const key=p=>p.join(',');let resolvedGoal=[...goal];const distance=p=>Math.abs(p[0]-resolvedGoal[0])+1.2*Math.abs(p[1]-resolvedGoal[1])+Math.abs(p[2]-resolvedGoal[2]);
 ...
 for(let n=0;n<4;n++){const floor=classify(resolvedGoal[0],resolvedGoal[1]-1,resolvedGoal[2]);if((floor.solid&&!floor.hazard)||classify(...resolvedGoal).water)break;resolvedGoal[1]--;}
 const open=[start],openKeys=new Set([key(start)]),score=new Map([[key(start),0]]),previous=new Map(),closed=new Set();let best=start,bestDistance=distance(start),expanded=0,reached=false;
 while(open.length&&expanded<maxNodes){
  let bi=0,bf=Infinity;for(let i=0;i<open.length;i++){const f=score.get(key(open[i]))+distance(open[i]);if(f<bf){bf=f;bi=i;}}
  const current=open.splice(bi,1)[0],ck=key(current);openKeys.delete(ck);if(closed.has(ck))continue;
  if(ck===key(resolvedGoal)){best=current;reached=true;break;}
  ...
  for(const [dx,dz] of [[1,0],[-1,0],[0,1],[0,-1]]){
   ...
   previous.set(nk,current);score.set(nk,ng);if(!openKeys.has(nk)){open.push(next);openKeys.add(nk);}
  }
 }
 ...
 path.reverse();return {path,reached,expanded,partial:!reached&&path.length>0,resolvedGoal};
}
