...
Сохранены существующие маршруты `/api/apps`, `/api/register`, `/api/login`, `/api/me`, `/api/logout` и прежний интерфейс `MiniSocket`. Для Voxel World добавлены маршруты `/api/voxel` и `/api/roblox-texture`. Игровые клиенты продолжают отправлять события `chat:*`, `survival:*` и `sharabass:*`; `shared/common.js` прозрачно переводит их в stateless API и Supabase Realtime.
