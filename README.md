...
Единый проект браузерных WebGL-приложений: 3D-каталог, глобальный чат, survival и World-Sharabass. Графика, управление и игровые механики сохранены; постоянные данные и realtime перенесены на Supabase, а HTTP API подготовлен для stateless-функций Vercel.
...
- Postgres хранит профили, игроков, инвентарь, мир, чат, ресурсы, постройки и объекты.
...
Сохранены существующие маршруты `/api/apps`, `/api/register`, `/api/login`, `/api/me`, `/api/logout` и прежний интерфейс `MiniSocket`. Игровые клиенты продолжают отправлять события `chat:*`, `survival:*` и `sharabass:*`; `shared/common.js` прозрачно переводит их в stateless API и Supabase Realtime.
