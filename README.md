...
**ВНО = Воспроизводимость, Независимость, Опровержение** (`Reproducibility, Independence, Falsification`). Это постоянный цикл `World_server`: открытие → следующий проверяемый вопрос → перенос в gameplay → отдельная кодовая проверка → усиление ВНО → улучшение метода → следующий цикл.

Каноническое описание: [`VNO.md`](VNO.md). Машинный индекс для любых AI/чатов: [`.ai/project-context-index.json`](.ai/project-context-index.json).
