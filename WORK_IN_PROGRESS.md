...
## 2026-09-26: Portable voxel AO and meshing primitives
Task: Independently implement portable, deterministic voxel-face ambient occlusion and greedy face merge primitives inspired by general techniques described in the upstream Godot/Unreal Minecraft project. Why: enable efficient lighting and geometry in Three.js and Godot without copying unlicensed source. Current state: existing World Server has its own PBR synthesis and world-quality scripts; prior PR #306 implements upload budgeting but not meshing. Target state: pure, renderer-agnostic AO/greedy meshing module with Node tests. Files: shared/voxel-face-mesher.mjs and test/voxel-face-mesher.test.mjs. Risks: face winding, AO diagonals, incorrect merging across materials or light values. Preserve: all Golden controls, collisions, APIs, graphics branches and backend. Regression prevention: deterministic order and no merging incompatible material/AO cells. Exact plan: standalone pure functions, tests and draft PR. Tests: node --test targeted, npm run check, independent graphics review. Deployment: no merge or deploy before gates. Progress: branch created; baseline not executable through GitHub connector; implementing isolated module. Next action: code, test and open PR. Completion: exact head plus CI and renderer integration evidence. Final evidence: pending.
