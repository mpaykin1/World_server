...
---
# 2026-09-26: Creature Factory policy-driven LOD runtime

Task: make the main-game Creature Factory consume the canonical LOD tier order and despawn semantics instead of hard-coded assumptions. Why: data/creature-lod-policy.json declares tierOrder and despawn, while shared/creature-visual-runtime.mjs currently hard-codes tier names and only hides low LOD. Current state: master 7b4564df00895842058762226587c44cf03a69a0; PR #310 remains staged/unmerged; PR #309 remains independent. Target: policy-driven tier selection, fail-safe policy validation, despawn/culling behavior, deterministic regression coverage without replacing the existing renderer. Files/systems: shared/creature-visual-runtime.mjs, test/creature-visual-lod-policy.test.mjs. Risks: malformed remote policy, empty tier order, resurrecting hidden creatures, changing combat/animation behavior. Preserve: current Three.js renderer, procedural silhouette style, pointer damage, deterministic spawn, existing defaults. Exact patch: store tierOrder; validate fetched tiers/order; select tiers in configured order; derive visibility from despawn; hide detail outside full; expose policy helpers for deterministic Node tests. Tests: focused Node test plus repository gates/CI; browser/mobile FPS remains separate evidence and must not be inferred. Deployment/PR: isolated branch -> PR -> Fleet PRE -> Ocean -> Fleet POST. Current progress: implementation in progress. Next action: patch runtime and add focused regression tests. Completion criteria: focused tests and CI pass at exact candidate SHA, then independent PRE. Final evidence: pending.
