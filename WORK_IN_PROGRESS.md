...
Task: repair false-positive HTTP 200 hosting/app smoke for Chain Reaction without touching Graphics, backend, Fleet POST or the independent-review infrastructure. Why: scripts/post-deploy-smoke.js accepted Site not found, app bootstrap loss, HTML in place of /api/apps, and cross-origin login redirect with status 200. Current state: protected master 809b0292b26edb1ea320e61144f1b1dfed772922; existing quality-canary already checks exact-SHA browser identity but its HTTP smoke has no such pin. Target: preserve one smoke script and one quality-canary workflow, fail closed on false-ready HTTP responses, and require exact Cloudflare identity in canary. Files/systems: scripts/post-deploy-smoke.js, test/post-deploy-smoke.test.js, .github/workflows/quality-canary.yml and this ledger. Risks: false positive success, false negatives from legitimate HTML, accidental claim of authenticated Fleet POST. Preserve: current public routes, Cloudflare identity helper, source-sha canary deployment and all existing Graphics/Builder/Fleet ownership. Patch: add real app identity/bootstrap and API JSON checks; reject host errors and cross-origin redirects; reuse existing Cloudflare exact-SHA identity gate on opt-in; pin the canary to github.sha; add deterministic offline mocks for healthy and defective responses. Tests: node --check, node --test test/post-deploy-smoke.test.js test/quality-canary-config.test.js, node scripts/check-agent-rules.js, git diff --check and exact-head cloud workflows. Deployment/PR: separate branch ai/chatgpt/chain-release-smoke-20260924, PR into master; no production deploy or Fleet POST by this task. Progress: isolated worktree outside Desktop; patch written; 12/12 targeted Node tests PASS, syntax/agent rules/diff check PASS. Next action: commit/push, open PR and collect exact-head cloud checks. Completion: no false HTTP smoke PASS for documented defective responses; exact deploy SHA checked by canary. Final evidence: local focused PASS; cloud and independent Fleet PRE PENDING, no production/authenticated/device/user-visible acceptance claim.
