# 2026-09-24: Certified four-card selection vs fifth free-design intent
Task: refresh existing PR #289 onto master `809b0292b26edb1ea320e61144f1b1dfed772922` after #286/#293 integration. A selected certified Genie card must retain its server-certified structure even if explanatory text mentions another project; the fifth `free-design` lane remains intention-driven. Scope: reuse the existing Node and Supabase Edge adapters and the one consequence engine; no Graphics files, new runtime or automation. Risks: stale/forged IDs, hidden-category leakage, dropping #285/#286 privacy/DTO behavior, or letting text silently switch a fixed card. Patch: validate the offered `choiceId` against current-revision `genieOptions`, lock the certified structure while retaining cautious/reckless modifiers, preserve backward compatibility without an ID, and keep free-design interpretation. A regression also proves that the specific phrase `солнечная электростанция` wins over the generic electricity token instead of being misclassified as geothermal. Tests: focused API/Edge/engine suites, adversarial stale/mismatch/free-design cases, Node/Edge parity, full cloud CI and independent exact-head Fleet PRE; Ocean and separate Fleet POST only after PASS. Progress: unique PR #289 delta is reconciled with fresh master; Graphics #278/#288 and optional AI PR #291 remain untouched. Completion: exact-head handoff on the existing PR with no inherited #286 diff. Final evidence: 65/65 focused tests PASS, including the actual imported Edge TypeScript adapter, stale/forged/mismatched IDs, fifth free design, resident/history privacy, membership/revoke, revision fence and two-writer CAS; syntax (67 files), agent rules and diff check PASS. Full local check reached 827 tests with 810 PASS and 4 intentional skips; 13 unrelated host-environment failures are caused by absent npm/Python dependencies (`@supabase/supabase-js`, `esbuild`, `requests`) and unchanged bridge/link fixtures. Exact-head cloud CI and independent Fleet PRE remain mandatory before Ocean.
