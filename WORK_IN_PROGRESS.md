# 2026-09-25: Free SimCity-inspired city-services slice (MANUAL_FAST_LANE)

Task: add a real deterministic household-level electricity/water/road availability projection to the EXISTING Chain Reaction consequence engine, inspired by Micropolis/LinCity/Citybound mechanics but copying NO GPL/AGPL code. User explicitly requires free alternatives to paid game systems. Current master 809b0292 has aggregate power/water, resident addresses, projects and hazard events, but no per-house service networks or free player-readable outage feedback. Target: bounded reproducible spanning-tree links between canonical houses, supply dispatch by floors, utility project backup anchors, hazard-broken links, equitable rotating priority during scarcity, aggregate commute reach, saved service projection and short offline Russian causal timeline; zero external paid service/API, CPU-only. Scope: supabase/functions/_shared/world-consequence-engine.js (shared Node+Edge implementation), dedicated regression test, docs; NO graphics files, API migrations, second simulator, paid model, new scheduled task, or direct master write. Ownership: distinct infrastructure slice; reconcile with open engine PRs #295/#296 and Graphics #278 before integration.

Risks: changing Genie counterfactual arithmetic/card categories, exposure of private actor/comment, breaking deterministic replay, expensive graph generation, legacy-world hydration, 1 MiB world state cap, PR overlap and misleading player-visible claims. Implementation: add pure bounded cityServices(world), compute initial projection at creation, refresh on existing authoritative tick; add service-change events ONLY for valid existing snapshots, keep aggregate resource arithmetic unchanged; produce free deterministic plain-language Genie card explanations. New PR must remain draft until focused/full cloud CI and genuine independent Fleet PRE on exact head; Ocean alone integrates/deploys, Fleet POST performs authenticated deployed simulation and real desktop/mobile visual proof. Do not give gameplay URL or claim visibility >85% from code alone. Tests: deterministic graph, zero-resource outages, backup resilience under hazard, scarcity fairness, legacy repair, offline card descriptions, Node/Edge parity via existing cloud suite, syntax, diff, agent rules and quality gates. Next: push small tested code/test/docs commit, obtain cloud exact-head evidence and Fleet PRE, address rebase/CI issues, integrate only after independent approval. Completion: merged + deployed + Fleet POST live + >85% measured visual acceptance; final evidence UNKNOWN until all gates pass.
