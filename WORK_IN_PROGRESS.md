...
## 2026-09-26 ChatGPT isolated macro-to-Chain-Reaction geography bridge
Task: Connect a player-placed volcano, forest, and ocean in the existing emergence map to the SAME persisted Chain Reaction geography, with Node/Edge parity.
Why: Current macro_place writes worldDNA.emergence only while game-state creates a randomized geography independently of the visible map; placed volcano can be rejected as absent.
Current state: protected master 809b0292b26edb1ea320e61144f1b1dfed772922, macro_place and Chain Reaction are separate fields; #299 owns core consequence engine/API and #278 owns UI.
Target state: successful macro placement atomically updates/initializes Chain Reaction land in the same row CAS, advances its revision only when physics-relevant geography changes, and never invents a volcanic resource.
Files / systems involved: NEW supabase/functions/_shared/chain-reaction-geography.js, api/voxel.js macro CAS writer, supabase/functions/world-emergence/index.ts macro CAS writer, NEW independent targeted tests.
Known risks: historical unmigrated worlds, guest map ownership, concurrent writers, Edge global module import, preexisting project intent; do not modify other agents' owned files.
Golden systems to preserve: single authoritative consequence engine, Node/Edge parity, resident DTO privacy, CAS, existing maps, deterministic ticks, FPS/WASD/mobile, 5 AKA automations.
Errors that must not return: world mutation without matching CAS, implicit instant power, fabricated offers, stale same-revision cards after a geography change.
Exact patch: add one pure shared geography bridge, call it only in both canonical macro CAS writers after emergence changes, add focused behavioral tests.
Tests to run: baseline Node/Edge/emergence; new bridge unit + Node actual handler + Edge import/behavior; npm run check, npm run quality:diff and release:gate if runtime permits; exact-head cloud CI and independent Fleet PRE.
Deployment / PR plan: isolated branch ai/chatgpt/geo-macro-bridge-20260926 from master; draft PR; no protected merge, Supabase deploy or preview release without independent exact-head PASS.
Current progress: fresh master and ownership checked, route/recall completed, clean separate off-Desktop worktree; baseline initially blocked by missing node_modules, npm ci started.
Next action: implement canonical bridge and run new focused regression against Node and Edge.
Completion criteria: placed volcano makes geothermal feasible in saved same-revision world, later macro changes invalidate stale expectedRevision; tests pass both runtimes, exact-head independent CI and live POST before release.
Final evidence (2026-09-26): 37/37 focused Node+actual Edge+macro+Causal tests PASS; 5 new regressions PASS; scripts/check-js.js 67 files PASS; agent rules PASS; git diff --check PASS. Full npm run check completed 903 tests: 900 PASS, 2 SKIP, 1 FAIL only in external live Ollama query (local model exceeded 45-second deadline while another larger model was active); full local gate NOT_VERIFIED/PARTIAL, cloud exact-head CI pending. No browser/real-device metric, authenticated live POST, independent final review or user visibility >85 claimed. Stable Cloudflare Worker read-only identity currently reports exact master 809b0292b26edb1ea320e61144f1b1dfed772922, distinct from stale world-server.ai.studio and UUID-only Git Integration previews. Legacy worlds with existing macro entities but no subsequent placement are not backfilled by this slice; Architect/Builder own first-load migration, graphics owner #278 own 3D results. Next: push isolated draft PR, independently verify exact-head, then protected merge, Supabase Edge rollout and authenticated desktop/mobile live tests.

### 2026-09-26 Follow-up after independent exact-SHA review (#312)
Independent review run 36209178505 of 82e0e5a6393e1f3e8d572161c9d36834c1441d5c returned BLOCK: Google 429, two reviewers invalid JSON/length, two other reviewers raised alleged 409 propagation and revision-change defects. Both alleged defects were investigated: `lib/http.js` `withErrors` maps thrown `.status` to HTTP 409 and `index.ts` Deno outer catch does likewise; geography-change comparison is nontrivial and independent world-state tests show the revision advances on a real land delta. To make these claims falsifiable, added actual Node HTTP 409 middleware and Edge error-path regressions and explicit forced land-delta/history assertions; marked corrupt scenario 409 non-retryable in Node to avoid wasted DB reads. Re-ran focused Node + Edge suite: 38/38 PASS. Do not treat old independent run as PASS; new HEAD requires independent exact-SHA re-review. Earlier PR SHA Cloudflare exact-head preview run 36209178448 delivered homepage, catalog, voxel app, config SHA and Chromium 2-player test; not real Supabase Edge candidate deploy, visual 85% metric, or production. New HEAD pending after commit.
