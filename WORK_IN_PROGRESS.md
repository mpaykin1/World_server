...
# 2026-09-24: Deterministic severe-crisis recovery cooperative

Task: add the missing achievable recovery branch for a world trapped at zero power, water and food. Why: the design requires continued play after catastrophe, but the current deterministic tick only removes population/health and can remain permanently stuck without enough resources to start a project. Base: master `809b0292b26edb1ea320e61144f1b1dfed772922`. Scope: the existing shared consequence engine and focused simulation tests only; no API route, UI, second engine, AI arithmetic, schema migration, deployment or automation. Target: after six consecutive crisis ticks, residents activate one canonical emergency cooperative that immediately restores a survival floor and then produces bounded essentials at explicit budget/ecology/culture/jobs/population tradeoffs; it phases out after six genuinely stable ticks or a hard 24-tick relief window, with at most three interventions. Risks: free-resource loops, duplicate intervention events, replay drift, malformed legacy state, accidental game-over or permanent subsidy. Patch plan: add validated recovery state, apply the cooperative's deterministic flow before project arithmetic, trigger once at the threshold, recompute crisis after intervention, and add exact-boundary/replay/tradeoff/phase-out tests. Required tests: focused engine/API parity regressions, syntax, agent rules, diff check, then exact-head cloud CI and independent Fleet PRE. Delivery: isolated branch and PR to master; Ocean only after PASS; Fleet POST after deployment. Progress: implementation and local falsification complete; the first independent PRE found a workshop-driven permanent subsidy and unsafe legacy counter overflow, both repaired with exact regressions. Completion: exact deterministic tests and protected integration handoff. Final evidence: 59/59 focused engine/API/actual Edge runtime tests PASS; workshop counterexample remains bounded across 2,000 ticks; unsafe legacy counters clamp without phantom events; 1,000 independent seeds × 120 ticks replay bit-for-bit with finite resources, population floor and intervention-spacing/count invariants; 67-file JS syntax, agent rules and diff check PASS. A fresh exact-head cloud/Fleet cycle is required before Ocean.
