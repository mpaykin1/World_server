...
## 2026-09-26: graphics-owned chunk upload scheduler
Owner: ChatGPT; isolated branch ai/chatgpt/voxel-stream-budget-20260926. Goal: original bounded mesh upload scheduler inspired by independently documented Godot chunk-streaming design; no external source/assets copied. Current: branch from master, implementation pending. Target: deterministic visible-first scheduling, revision invalidation and bounded work per frame. Files: new graphics utility and Node tests only. Risks: stale chunk uploads and starvation. Plan: pure module + tests; run node --test and npm run check, request independent Fleet PRE; no direct master push/merge/deploy. Next: implement scheduler and tests. Completion evidence: exact SHA, test output, independent review; live verification NOT_VERIFIED.
