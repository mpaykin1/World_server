...
const report={schemaVersion:'4.0.0',system:'WORLD_RETARGET_CONTRACT',generatedAt:new Date().toISOString(),boneAliases:aliases,constraints,runtimeSamples:samples.length,mappedRuntimeSamples:mapped,contractReady:true,runtimeEvidenceReady:samples.length>0&&mapped===samples.length,note:samples.length?'Runtime skeleton evidence evaluated.':'Universal semantic contract is installed; real rig evidence is still required for 100% animation.'};
fs.writeFileSync(path.join(ROOT,'WORLD_RETARGET_CONTRACT_REPORT.json'),JSON.stringify(report,null,2)+'\n');
console.log(`[WORLD_RETARGET_CONTRACT_V4] contract ready · runtime samples=${samples.length}`);
