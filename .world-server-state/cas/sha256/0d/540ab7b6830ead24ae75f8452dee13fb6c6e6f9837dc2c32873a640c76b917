...
begin
  ...
  if p_job_type is null or p_job_type not in ('WORLD_REGION','CHARACTER_CREATE','ASSET_CREATE','MECHANIC_CREATE','CODEX_AUDIT') then
...
revoke all on function public.create_private_factory_request(uuid,text,uuid,text,text,jsonb) from public;
revoke all on function public.create_private_factory_request(uuid,text,uuid,text,text,jsonb) from anon;
revoke all on function public.create_private_factory_request(uuid,text,uuid,text,text,jsonb) from authenticated;
