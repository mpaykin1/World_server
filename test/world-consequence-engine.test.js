...
test('catastrophe does not end the simulation and opens a recovery branch',()=>{let w=E.createWorld('crisis');w.resources.power=0;w.resources.food=0;for(let i=0;i<15;i++)w=E.tick(w);assert(w.population>0);assert.equal(w.tick,15);assert(w.history.some(x=>x.kind==='adaptation'));assert(w.history.some(x=>x.kind==='crisis_recovery_started'));assert(w.recovery.interventions>=1)});
