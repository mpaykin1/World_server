...
test('V8 physical report API and DB migration exist',()=>{for(const p of ['lib/api-handlers/procedural-quality-device-report.js','lib/api-handlers/procedural-quality-certification.js','supabase/procedural_quality_v8.sql'])assert.ok(fs.existsSync(path.join(root,p)),p)});
test('V8 profile exposes true velocity, voxel radiance and device certification',()=>{const p=require('../lib/api-handlers/procedural-quality-profile.js').buildProfile({webgpu:'1',webgl2:'1',memory:16,cores:16,dpr:1});assert.ok(p.version>=8);assert.equal(p.policy.trueSkinnedVelocity,true);assert.equal(p.policy.voxelSceneRadiance,true);assert.equal(p.policy.physicalDeviceCertification,true)});
