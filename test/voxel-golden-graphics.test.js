...
test('voxel world has baked vertex AO in chunk meshing',()=>{
  ...
  assert.ok(/vertexShade\?\.\[i\]/.test(src) || [0,1,2,3].every(i=>src.includes(`faceColors[vertexShade?.[${i}]??0]`)||src.includes(`faceColors[vertexShade[${i}]]`)), 'AO shade lookup must cover all four face vertices');
