...
const canonicalExists = fs.existsSync(path.join(canonical, '.git'));

test('wrong workdir / unicode path auto-finds canonical World_server', { skip: !canonicalExists && 'this project\'s desktop-ai session recovery targets a fixed local desktop path (~/Desktop/World_server) that does not exist on CI runners or other machines - only meaningful on the primary dev machine' }, () => {
