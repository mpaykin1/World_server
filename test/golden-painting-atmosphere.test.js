...
test('golden painting runtime is loaded by every playable web world',()=>{
  const worlds=['ai3d-voxel-city','dark-void-scene','survival','voxel-world','world-sharabass'];
...
test('Three.js worlds register real scene atmosphere',()=>{
  for(const world of ['ai3d-voxel-city','dark-void-scene','survival','voxel-world']){
...
test('Sharabass custom shader receives time state and aerial perspective',()=>{
  ...
  assert.match(html,/smoothstep\(MAX_DIST \* 0\.32, MAX_DIST, t\)/);
