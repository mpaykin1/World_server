...
// No global OpenCode installation or hosted call is needed by this fixture.
const resolver = test.mock.method(cp, 'spawnSync', (command,args,opts) => {
  if (['where','which'].includes(command) && args[0] === 'opencode') return { status: 0, stdout: path.join(root,'fixture-opencode.exe'), stderr: '' };
...
test('Zero-Chaos Git readiness rejects dirty, untracked and unpublished work idempotently',()=>{
  ...
  git(['branch','preserved-unpublished','HEAD'],repo);
  git(['checkout','--detach','refs/remotes/origin/main'],repo);
  const preserved=housekeeping.gitZeroChaosReadiness(repo);
  assert.equal(preserved.worktrees[0].unpublishedCommits,0);
  assert.equal(preserved.unpublishedBranchCommits,1);
  assert.equal(preserved.verdict,'FAIL','unpublished branches survive worktree cleanup');
...
test('reviewed source mentioning permission errors does not itself count as a refusal',async t=>{
  ...
  assert.equal(r.result,'PENDING_REVIEW');assert.equal(r.verified,false);assert.equal(r.stdout,'Review complete: no findings.');
});
...
test('worker self-reported failure cannot become overall PASS',async t=>{
  worker(t,()=>({status:0,stderr:'',stdout:'Task failed; no result produced.'}));
  const r=await mc.invokeOpencode('fixture',{taskId:'semantic-failure',push:false});
  assert.equal(r.result,'PENDING_REVIEW');
  assert.equal(mc.summarizeMasterResults([r]),'PENDING');
  const records=fs.readFileSync(process.env.AI_AGENT_REPORTS_PATH,'utf8').trim().split('\n').map(JSON.parse);
  assert.ok(records.some(x=>x.task_id==='semantic-failure'&&x.status==='review_required'));
});
test('trusted artifact validator is required for verified PASS',async t=>{
  worker(t,()=>({status:0,stderr:'',stdout:'Reviewed fixture'}));
  const r=await mc.invokeOpencode('fixture',{taskId:'verified-artifacts',push:false,validateResult: async candidate=>{
    assert.ok(fs.existsSync(candidate.worktree));
    assert.equal(candidate.stdout,'Reviewed fixture');
    return {ok:true,evidence:['Fixture output and checkout verified by caller']};
  }});
  assert.equal(r.result,'PASS');assert.equal(r.verified,true);
});
test('existing task checkout is queued for recovery without losing contents or throwing',async()=>{
  const existing=mc.createIsolatedWorktree('interrupted-task');
  fs.writeFileSync(path.join(existing.dir,'unique.txt'),'interrupted work');
  const r=await mc.invokeOpencode('fixture',{taskId:'interrupted-task',push:false});
  assert.equal(r.result,'RECOVERY_REQUIRED');
  assert.equal(fs.readFileSync(path.join(existing.dir,'unique.txt'),'utf8'),'interrupted work');
  assert.equal(mc.summarizeMasterResults([r]),'PENDING');
});
test('shared report persistence failure cannot be silently accepted',async t=>{
  worker(t,()=>({status:0,stderr:'',stdout:'Review completed'}));
  const append=fs.appendFileSync;
  t.mock.method(fs,'appendFileSync',(file,...args)=>{
    if(file===process.env.AI_AGENT_REPORTS_PATH)throw Error('report disk full');
    return append(file,...args);
  });
  const r=await mc.invokeOpencode('fixture',{taskId:'report-failure',push:false});
  assert.equal(r.result,'RECOVERY_REQUIRED');assert.equal(r.reportedToSharedPipeline,false);
});
