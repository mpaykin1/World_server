...
test('ScienceGameplayAdapter loads contract and checks gates', () => {
  const contract = adapter.loadContract('RUN_072');
  assert.equal(contract.runId, 'RUN_072');
  assert.equal(contract.mechanic.type, 'redundant_cycle_closure');
...
test('RUN_072 ignores natural terrain and tiny player structures', () => {
