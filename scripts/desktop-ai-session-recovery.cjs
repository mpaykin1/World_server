...
function findRoot(start = process.cwd()) {
  ...
  while (true) {
    ...
    if (parent === cur) throw new Error('Cannot find repository root (package.json).');
    cur = parent;
  }
}
...
function policy() {
  return Object.assign({
    ...
    processMarkers: ['long-soak-runner.cjs','autonomous-blocker-repair.cjs','quality_autoloop','toolchain-bootstrap.cjs','vercel inspect','npm run blockers:'],
...
function futureTimer(scan) {
  const times=[];
  if (scan?.blockerRepair?.nextRunAt) times.push(scan.blockerRepair.nextRunAt);
  for (const x of scan?.items||[]) if (x?.details?.nextRunAt) times.push(x.details.nextRunAt);
  const valid=times.map(x=>({raw:x,t:new Date(x).getTime()})).filter(x=>Number.isFinite(x.t)&&x.t>Date.now()).sort((a,b)=>a.t-b.t);
...
function classifySessionHealth(s, scan, processes, primary, prev) {
  ...
  const timer=futureTimer(scan);
  const hasUnfinished=Boolean((scan?.items||[]).length);
  const runningKnown=(scan?.items||[]).some(x=>x.status==='running');
  let status='LIVE', reason='recent recovery activity';
  if (!s || (s.status==='finished' && !hasUnfinished)) { status='IDLE'; reason='no active recovery session or unfinished work'; }
  ...
  else if (activeCommand && !commandActive && activityMinutes >= Number(p.liveActivityMinutes||3)) { status='DEAD'; reason='active command was recorded but no matching OS process remains'; }
  ...
  else if (timer && !((scan?.items||[]).some(x=>x.blocking && !['waiting','running'].includes(x.status)))) { status='WAITING_VALID'; reason=`durable timer is pending until ${timer}`; }
  else if (runningKnown && !changed) { status='WAITING_VALID'; reason='known managed background process is still running'; }
  else if (changed || activityMinutes < Number(p.liveActivityMinutes||3)) { status='LIVE'; reason=changed?'observable durable progress changed since previous watchdog tick':'recent session activity'; }
  else if ((primary.alive || commandActive) && activityMinutes >= Number(p.stalledMinutes||12)) { status='STALLED'; reason=`responsible process is alive but no durable progress was observed for ${activityMinutes.toFixed(1)} minute(s)`; }
  else if (!primary.alive && !commandActive && hasUnfinished && activityMinutes >= Number(p.stalledMinutes||12)) { status='DEAD'; reason=`unfinished work exists but no responsible process is alive after ${activityMinutes.toFixed(1)} minute(s)`; }
  else { status='LIVE'; reason='within stall grace period; continue observation'; }
  return { status, reason, fingerprint, changed, activityMinutes:Number(activityMinutes.toFixed(2)), commandActive, waitMarker, nextTimerAt:timer };
}
...
function commandWatchdog(args={}) {
  ...
  try {
    ...
    const health={ schemaVersion:1, engineVersion:VERSION, checkedAt:nowIso(), status:c.status, reason:c.reason,
      progressChanged:c.changed, activityMinutes:c.activityMinutes, nextTimerAt:c.nextTimerAt, primaryLock:primary,
      activeCommand:s?.activeCommand||null, commandProcessAlive:c.commandActive, unfinishedCounts:scan.counts,
      sessionId:s?.sessionId||null, task:s?.task||null, nextAction:s?bestNextAction(s,scan):null,
      recoveryCheckpointCreated:false, safeToStartNewChat:['STALLED','DEAD'].includes(c.status) };
    ...
    if(['STALLED','DEAD'].includes(health.status)) writeRecoveryRequest(health);
