...
try{let out;if(cmd==='enqueue')out=enqueue(a[0],a[1]||'{}',a[2]||0,a[3]||3,a[4]);else if(cmd==='claim')out=claim(a[0]||`worker-${process.pid}`,a[1]||300000);else if(cmd==='heartbeat')out=heartbeat(a[0],a[1],a[2]||'{}',a[3]||300000);else if(cmd==='ack')out=ack(a[0],a[1],a[2]||'{}');else if(cmd==='fail')out=fail(a[0],a[1],a[2]||'error',a[3]||5000);else if(cmd==='resume-stale')out=resumeStale(a[0]||0);else if(cmd==='retry-dead')out=retryDead(a[0]);else if(cmd==='events')out=events(a[0]);else if(cmd==='health')out=health();else throw new Error('usage: durable-job-queue.cjs health | enqueue <kind> <json> [priority] [maxAttempts] [dedupeKey] | claim [worker] [leaseMs] | heartbeat <id> <worker> <checkpoint-json> [leaseMs] | ack <id> <worker> <result-json> | fail <id> <worker> <error> [retryDelayMs] | resume-stale [graceMs] | retry-dead [id] | events <id>');console.log(JSON.stringify(out,null,2))}catch(e){console.error('[DURABLE_QUEUE] FAIL',e.stack||e.message);process.exit(2)}
