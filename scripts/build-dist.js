...
function buildDist(targetRootDir) {
  ...
  // Ensure root index.html exists in dist (fallback to apps/catalog/index.html)
  const distIndexHtml = path.join(distDir, 'index.html');
  if (!fs.existsSync(distIndexHtml)) {
    const catalogIndexHtml = path.join(rootDir, 'apps', 'catalog', 'index.html');
    if (fs.existsSync(catalogIndexHtml)) {
      fs.copyFileSync(catalogIndexHtml, distIndexHtml);
    }
  }
