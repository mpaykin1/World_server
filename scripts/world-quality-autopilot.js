...
const status={schemaVersion:'4.1.0',generatedAt:new Date().toISOString(),readinessPercent:report.readinessPercent,domainPercent:report.domainPercent,hardGateReady:report.hardGateReady,runtimeQualityPercent:runtime?.percent??null,deviceEvidencePercent:device?.percent??null,evidenceLedgerSha256:ledger?.ledgerSha256??null,materialProfiles:materials?.profiles??null,materialForge:materials?.materialForge??null,visibilityPercent:visibility?.percent??null,microdetailPercent:microdetail?.percent??null,microdetailSchemaVersion:microdetail?.schemaVersion??null,retargetContractReady:retarget?.contractReady??false,schedulerRoutes:scheduler?.routes?.length??0,candidateCount:lab?.candidateCount??0,feedbackSamples:learner?.samples??0,nextSystems:(plan.actions||[]).slice(0,8),mode:args.has('--evolve')?'candidate-evolution':args.has('--full')?'full-diagnostics':'safe-analysis'};
