...
const report=read('WORLD_QUALITY_AUTOPILOT_REPORT.json')||{},plan=read('WORLD_QUALITY_AUTOPILOT_PLAN.json')||{},runtime=read('WORLD_RUNTIME_QUALITY_REPORT.json'),device=read('WORLD_DEVICE_PROFILE_MATRIX.json'),ledger=read('WORLD_QUALITY_EVIDENCE_LEDGER.json'),materials=read('WORLD_MATERIAL_SYNTHESIS_REPORT.json'),visibility=read('WORLD_VISIBILITY_OPTIMIZER_REPORT.json'),retarget=read('WORLD_RETARGET_CONTRACT_REPORT.json'),scheduler=read('WORLD_QUALITY_SCHEDULER_REPORT.json'),lab=read('WORLD_CANDIDATE_LAB_REPORT.json'),learner=read('WORLD_FEEDBACK_LEARNER_REPORT.json');
const status={schemaVersion:'4.0.0',generatedAt:new Date().toISOString(),readinessPercent:report.readinessPercent,domainPercent:report.domainPercent,hardGateReady:report.hardGateReady,runtimeQualityPercent:runtime?.percent??null,deviceEvidencePercent:device?.percent??null,evidenceLedgerSha256:ledger?.ledgerSha256??null,materialProfiles:materials?.profiles??null,visibilityPercent:visibility?.percent??null,retargetContractReady:retarget?.contractReady??false,schedulerRoutes:scheduler?.routes?.length??0,candidateCount:lab?.candidateCount??0,feedbackSamples:learner?.samples??0,nextSystems:(plan.actions||[]).slice(0,8),mode:args.has('--evolve')?'candidate-evolution':args.has('--full')?'full-diagnostics':'safe-analysis'};
