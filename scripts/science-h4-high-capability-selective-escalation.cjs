...
const R=process.cwd(),SEED='63063',BASE='qwen3-fast:1.7b',SPEC='qwen3:4b';
...
if(require.main===module)(async()=>{
 ...
 for(let i=0;i<tasks.length;i++){
  ...
  console.log('[RUN_063]',i+1,e.id,'esc='+g.escalate,'conf='+g.confidence,'q',rows.at(-1).baseline.q.toFixed(3),'->',rows.at(-1).final.q.toFixed(3),'ms',base.ms,'->',rows.at(-1).collectiveMs);
 }
 ...
 const report={experiment:'RUN_063_H4_BLIND_HIGH_CAPABILITY_SELECTIVE_ESCALATION',preregisteredBeforeExecution:true,seed:SEED,bestSingle:BASE,specialist:SPEC,sharedMemory:'same top-2 lexical retrieval from COLLECTIVE_BRAIN_CONTEXT.md for both arms',priorResultsPreserved:'RUN_055 FAIL and RUN_056 FAIL remain unchanged; all prior H4 task IDs are excluded using existing RUN_* evidence from the prior science worktree',subhypothesis:'Escalating only very-low-confidence best-single outputs to a stronger 4B specialist can improve multiobjective utility while keeping specialist usage a minority.',taskSelection:'8 deterministic unseen registry tasks excluding all task IDs found in prior RUN_*_H4_*.json',gate:'observable baseline only: first-line confidence <=0.30 OR invalid confidence/structure; scorer never routes or selects',confirmation:'1..3 escalations of 8; utility +0.03; q/n non-worse; errors non-higher; wall <=1.75x; >=1 escalated task utility improvement >=0.03',refutation:'any confirmation component fails; preserve result and do not relax thresholds',taskIds:tasks.map(x=>x.id),rows,baseline:B,collective:C,delta:d,executionPass,hypothesisPass,pass:executionPass&&hypothesisPass};
 fs.writeFileSync(path.join(R,'RUN_063_H4_BLIND_HIGH_CAPABILITY_SELECTIVE_ESCALATION.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({taskIds:report.taskIds,baseline:B,collective:C,delta:d,executionPass,hypothesisPass,pass:report.pass},null,2));process.exitCode=executionPass?0:1
