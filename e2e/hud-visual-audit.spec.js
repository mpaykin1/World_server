...
const {decorativeFxIds}=require('./helpers/decorative-fx');
...
for(const app of apps){
  test(`hud-audit ${app}`,async({page})=>{
    ...
    const decorative=await page.evaluate(decorativeFxIds);
    const report=await page.evaluate((decorative)=>{
      ...
      for(const el of document.querySelectorAll('body *')){
        ...
        // Known noninteractive FX are scene composition; bounds and pixel gates still apply.
        if(ratio>.08&&!decorative.includes(el.id))issues.push({type:'large-persistent-overlay',id:el.id||el.className,ratio});
...
test('decorative FX contract rejects interactive or opaque impostors',async({page})=>{
  await page.setContent('<div id="phaser4FxLayer" aria-hidden="true" style="pointer-events:none"><canvas></canvas></div><div id="goldenCinematicPost" aria-hidden="true" style="pointer-events:none;opacity:.24"></div><div id="ordinaryPanel" style="pointer-events:none;position:fixed;inset:0">HUD</div>');
  expect(await page.evaluate(decorativeFxIds)).toEqual(['phaser4FxLayer','goldenCinematicPost']);
  for(const id of ['phaser4FxLayer','goldenCinematicPost']){
    await page.locator(`#${id}`).evaluate(el=>el.style.pointerEvents='auto');
    expect(await page.evaluate(decorativeFxIds)).not.toContain(id);
    await page.locator(`#${id}`).evaluate(el=>{el.style.pointerEvents='none';el.appendChild(document.createElement('button'));});
    expect(await page.evaluate(decorativeFxIds)).not.toContain(id);
    await page.locator(`#${id} button`).evaluate(el=>el.remove());
    await page.locator(`#${id}`).evaluate(el=>el.style.backgroundColor='black');
    expect(await page.evaluate(decorativeFxIds)).not.toContain(id);
  }
});
