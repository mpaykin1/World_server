...
function evaluateVisualCorrection(pipeline, input = {}) {
  ...
  if (stage.attempts >= pipeline.maxCorrectionPasses) { stage.status='blocked'; stage.blockers=['correction-budget-exhausted']; return pipeline; }
  stage.attempts += 1;
  const identity=Math.max(0,Math.min(1,Number(input.identityFidelity)||0)), hero=Math.max(0,Math.min(1,Number(input.heroDetailFidelity)||0)), structural=Math.max(0,Math.min(1,Number(input.structuralFidelity)||0));
  ...
  stage.evidence=[{type:'reference-runtime-screenshots',reference:referenceEvidence,runtime:runtimeEvidence,pass:evidencePass},{type:'visual-fidelity',identity,heroDetail:hero,structural,score,minIdentity,minHero,pass:fidelityPass}];
