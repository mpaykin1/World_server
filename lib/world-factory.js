...
const { createEmergenceStateFromIdea } = require('./world-emergence');
...
function evaluateVisualCorrection(pipeline, input = {}) {
  if (!pipeline?.stages?.length) throw new TypeError('reference pipeline required');
  const prior=pipeline.stages[6], stage=pipeline.stages[7];
  if (prior.status !== 'complete') { stage.status='blocked'; stage.blockers=['runtime-optimization']; return pipeline; }
  if (stage.status === 'complete') return pipeline;
  if (stage.attempts >= pipeline.maxCorrectionPasses) { stage.status='blocked'; stage.blockers=['correction-budget-exhausted']; return pipeline; }
  stage.attempts += 1;
  const autoFidelity = input.referenceImage && input.runtimeImage ? compareReferenceRuntime(input.referenceImage, input.runtimeImage, { heroMask: input.heroMask }) : null;
  const identity=Math.max(0,Math.min(1,Number(autoFidelity?.identityFidelity ?? input.identityFidelity)||0)), hero=Math.max(0,Math.min(1,Number(autoFidelity?.heroDetailFidelity ?? input.heroDetailFidelity)||0)), structural=Math.max(0,Math.min(1,Number(autoFidelity?.structuralFidelity ?? input.structuralFidelity)||0));
  const referenceEvidence=Boolean(input.referenceScreenshot), runtimeEvidence=Boolean(input.runtimeScreenshot);
  const minIdentity=Math.max(.85,Math.min(1,Number(input.minIdentityFidelity)||.9)), minHero=Math.max(.85,Math.min(1,Number(input.minHeroDetailFidelity)||.9));
  const score=Number((identity*.5+hero*.3+structural*.2).toFixed(4)), evidencePass=referenceEvidence&&runtimeEvidence, fidelityPass=identity>=minIdentity&&hero>=minHero&&score>=.85;
  stage.evidence=[{type:'reference-runtime-screenshots',reference:referenceEvidence,runtime:runtimeEvidence,pass:evidencePass},{type:'visual-fidelity',identity,heroDetail:hero,structural,score,minIdentity,minHero,method:autoFidelity?.method || 'provided-metrics',pixelsCompared:autoFidelity?.pixelsCompared || 0,pass:fidelityPass}];
  pipeline.visualCorrection={identityFidelity:identity,heroDetailFidelity:hero,structuralFidelity:structural,score,correctionPass:stage.attempts,referenceScreenshot:String(input.referenceScreenshot||'').slice(0,512),runtimeScreenshot:String(input.runtimeScreenshot||'').slice(0,512)};
  const pass=evidencePass&&fidelityPass; stage.status=pass?'complete':'needs-correction'; stage.blockers=pass?[]:[!evidencePass?'missing-reference-runtime-screenshot-evidence':'visual-fidelity-below-contract']; stage.completedAt=pass?new Date().toISOString():null;
  return pipeline;
}
...
function createWorldDNA({ idea, requestId, loreBible } = {}) {
  ...
  const emergence = createEmergenceStateFromIdea({ idea: clean, seed });
