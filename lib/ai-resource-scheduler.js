...
const QUEUE_SCRIPT = path.join(MAIN_TREE_ROOT, 'scripts', 'durable-job-queue.cjs');
...
function findLighterCandidate(capabilityClass, currentModel) {
  const { candidatesFor, isSuitable, scoreBackend } = require('./model-suitability');
  const registry = (() => { try { return JSON.parse(require('fs').readFileSync(require('path').join(__dirname, '..', 'data', 'model-registry.json'), 'utf8')); } catch { return { models: {} }; } })();
  const candidates = candidatesFor(capabilityClass);
  ...
  const lighter = candidates
    ...
    .filter((m) => isSuitable(m, capabilityClass).suitable)
    .sort((a, b) => registry.models[a].sizeGb - registry.models[b].sizeGb);
  if (!lighter.length) return null;
  return { model: lighter[0], sizeGb: registry.models[lighter[0]].sizeGb, score: scoreBackend(lighter[0], capabilityClass) };
}
...
async function decide(task = {}, opts = {}) {
  ...
  if (overRam) {
    ...
    const lighter = task.capabilityClass ? findLighterCandidate(task.capabilityClass, task.currentModel) : null;
    if (lighter) {
      action = 'run_now';
  ...
  return { action, reason, recommendedModel, resources, ollama, thresholds: THRESHOLDS };
}
...
function enqueueTask(payload, opts = {}) {
  const args = ['enqueue', 'anythingllm-task', JSON.stringify(payload), String(opts.priority || 0), String(opts.maxAttempts || 3)];
...
module.exports = { getResourceState, getOllamaState, decide, registerLiveWorker, enqueueTask, claimTask, ackTask, failTask, THRESHOLDS, QUEUE_DB };
