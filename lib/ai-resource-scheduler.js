...
function detectMainTreeRoot() {
  if (process.env.WORLD_SERVER_MAIN_TREE) return path.resolve(process.env.WORLD_SERVER_MAIN_TREE);
  const localRoot = path.resolve(__dirname, '..');
  try {
    const commonGitDir = cp.execFileSync('git', ['rev-parse', '--path-format=absolute', '--git-common-dir'], {
      cwd: localRoot, encoding: 'utf8', windowsHide: true,
    }).trim();
    return commonGitDir ? path.dirname(path.resolve(commonGitDir)) : localRoot;
  } catch {
    return localRoot;
  }
}
const MAIN_TREE_ROOT = detectMainTreeRoot();
...
function deferTask(id, worker, reason, retryDelayMs = 30000) {
  return queueCli('defer', id, worker, String(reason), String(retryDelayMs));
}

module.exports = { getResourceState, getOllamaState, decide, registerLiveWorker, enqueueTask, claimTask, ackTask, failTask, deferTask, THRESHOLDS, QUEUE_DB };
