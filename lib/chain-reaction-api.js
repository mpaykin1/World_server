...
function intentFrom(body, world) {
  ...
  // A certified card must keep its offered structure, even if commentary mentions another energy source.
  if (body.choiceId !== undefined && (typeof body.choiceId !== 'string' || body.choiceId.length > 128)) throw httpError(400, 'Invalid choiceId');
  ...
  const certified = engine.genieOptions(world).cards.find(card => card.id === body.choiceId);
  if (!certified || certified.structure !== body.structure) throw httpError(409, 'STALE_CHOICE');
...
function publicState(value) {
  ...
  // Rebuild from canonical world inputs. Allowlisting keys alone is insufficient:
  // an attacker could hide private objects inside an otherwise allowed field.
...
function publicResident(resident) {
  if (!resident || typeof resident.id !== 'string' || typeof resident.name !== 'string' ||
      ...
      !Number.isInteger(resident.floor) || !Number.isInteger(resident.flat)) {
    throw httpError(409, 'Invalid resident data');
  }
  return {
    id: resident.id,
    name: resident.name,
...
function publicResidents(world) {
  if (typeof world.seed !== 'string' || !Array.isArray(world.houses)) throw httpError(409, 'Invalid resident directory');
  const houses = world.houses.map(house => {
    if (!house || typeof house.id !== 'string' || !/^[a-zA-Z0-9_-]{1,80}$/.test(house.id) ||
        !Number.isInteger(house.floors) || house.floors < 1 || house.floors > 100) {
      throw httpError(409, 'Invalid resident directory');
    }
...
module.exports = { ACTIONS, handle, validateBody, hasMembership, membershipRole, manageMembership, publicState, residentAddress, publicResident, publicResidents };
