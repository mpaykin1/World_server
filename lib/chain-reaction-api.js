...
async function handle(admin, req, body) {
  ...
  if (body.action === 'genie-ai-status') return { ...base, ...genieAI.status(process.env.OPENAI_API_KEY) };
  if (body.action === 'genie-narrate') return {
    ...base,
    ...(await genieAI.narrate({ engine, world, structure: body.structure, text: body.text,
      actorId: authUser.id, apiKey: process.env.OPENAI_API_KEY }))
  };
