...
// Point 3 (this new cycle): a generic bug-description word ("broken",
// "issue", "wrong") is common across many unrelated test/file names in
// this repo (test/generated/*-broken.test.js exists for several different
// real bugs) - a single such word meeting scoreCandidate's own >=6-char
// "specific" weight was found live to admit an unrelated file into a
// context slot on real evidence-check (a "fix a broken navigation link"
// goal pulled in stairs/wall collision test files via "broken" alone).
// These words describe the TASK, not the TARGET, and are excluded the
// same way "fix"/"add"/"edit" already were.
const STOPWORDS = new Set(['the','a','an','is','are','was','were','be','to','of','in','on','for','and','or','it','this','that','so','so-that','with','without','not','same','style','as','other','apps','app','file','only','exactly','single','line','contains','content','add','fix','edit','change','update','make','sure','line-that','tag','broken','bug','issue','issues','wrong','invalid','incorrect','error','errors','missing','problem']);
