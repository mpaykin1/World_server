...
async function handleCreate(admin, identity, body) {
  ...
  if (existing) {
    ...
    if (error) throw error;
    return { mergeId: existing.id, resultWorldId: existing.result_world_id, spec: world.spec, reused: true, playUrl: playUrlFor(existing.result_world_id) };
  }
  ...
  // Provisioned regardless of who ultimately wins the merges race below --
  // every "worlds" row this handler ever creates (including an orphaned
  // loser's) stays fully playable, not just the canonical one.
  const { error: voxelError } = await admin.from('voxel_worlds').insert(deriveVoxelWorld(mergedSpec, resultWorldId));
  ...
  if (upserted) {
    return { mergeId: upserted.id, resultWorldId: upserted.result_world_id, spec: mergedSpec, reused: false, playUrl: playUrlFor(upserted.result_world_id) };
  }
  ...
  return { mergeId: winner.id, resultWorldId: winner.result_world_id, spec: winnerWorld.spec, reused: true, playUrl: playUrlFor(winner.result_world_id) };
}
...
module.exports._private = { newWorldId, sortedKey, playUrlFor, handleCreate, findExistingMerge, fetchPublishedWorlds };
