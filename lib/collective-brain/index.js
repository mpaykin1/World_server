...
function protectedLessonPayload(e){const content=safeText(`[${e.id}] Root cause: ${e.rootCause||e.symptom||'unknown'}. Solution: ${e.solution||'preserve existing regression protection'}`,5000),context=safeText(`Protection: ${(e.protection||[]).join('; ')}\nEvidence: ${(e.evidence||[]).join('; ')}`,4000);return{content,context,confidence:.9,project:'World_server',tags:['world-server','root-cause','regression-protection',String(e.category||'quality')].filter(Boolean)};}
