...
async function cycle(root=process.cwd(),opts={}){
  ...
  try{
    ...
    const status=sync==='queued'||lessonSync.failed.length?'DEGRADED':'PASS',report={patch:PATCH_ID,status,sync,hash:snapshot.contentHash,health:healthResult,outbox:flush,lessonSync,eventChain:verifyEventChain(root),generatedAt:nowIso()};writeJson(path.join(root,'COLLECTIVE_BRAIN_REPORT.json'),report);if((opts.requireAgentmemory||process.env.COLLECTIVE_BRAIN_REQUIRE_AGENTMEMORY==='1')&&status==='DEGRADED')throw new Error(`agentmemory required but degraded: ${healthResult?.error||state.lastError||lessonSync.failed[0]?.error||'unknown error'}`);return report;
