...
async function implementGoal({ mainRoot, goal, targetWorktree, models = [...OLLAMA_PATCH_MODELS, ...FREE_OPENCODE_MODELS], timeoutMs = 240000, verifyScript = 'check', maxContextLevel = 3 } = {}) {
  ...
  const orderedModels = localHasHistory ? ranking.order : [...models.filter((m) => m.startsWith('ollama:')), ...ranking.order.filter((m) => !m.startsWith('ollama:'))];
  ...
  for (const model of orderedModels) {
    ...
    if (!isLocal && !opencodeOk) {
      attempts.push({ model, level: null, classification: 'unavailable', ok: false, durationMs: 0, note: 'opencode CLI not available on this host' });
      continue;
    }
    if (!isLocal && openaiSkippable) {
      attempts.push({ model, level: null, classification: 'health_skip', ok: false, durationMs: 0, note: `skipped: ${openaiHealth.state}` });
      continue;
    }
    ...
    if (isLocal) {
      ...
      if (ollamaHealth.state === 'OLLAMA_UNAVAILABLE' || ollamaHealth.state === 'OLLAMA_OOM') {
        attempts.push({ model, level: null, classification: 'health_skip', ok: false, durationMs: 0, note: `skipped: ${ollamaHealth.state} (${ollamaHealth.reason})` });
        continue;
      }
    }
    ...
    for (let level = 1; level <= levelsForThisModel; level++) {
      ...
      const attempt = await resourceScheduler.withResourceSlot(mainRoot, resourceCommand, taskId, () => (
        isLocal
          ...
          ? invokeOllamaPatchOnce(localModelName, goal, targetWorktree, { timeoutMs: Math.max(levelTimeout, 120000), verifyScript, scopedFiles: ctx.files })
          : invokeOpencodeOnce(model, goal, targetWorktree, { timeoutMs: levelTimeout, verifyScript, scopedFiles: ctx.full ? [] : ctx.files })
      ), { maxWaitMs: 120000 });
...
module.exports = {
  ...
  runWithTreeKill, killTree, taskFingerprint,
