...
// think:false вЂ” see the identical, longer comment on
...
async function invokeOpencodeOnce(model, goal, targetWorktree, { timeoutMs = 240000, verifyScript = 'check', scopedFiles = [] } = {}) {
  const goalFile = path.join(sessionGuard.ensureRoots().scratchRoot, `agent-invoke-goal-${process.pid}-${Date.now()}.txt`);
...
async function implementGoal({ mainRoot, goal, targetWorktree, models = [...OLLAMA_PATCH_MODELS, ...FREE_OPENCODE_MODELS], timeoutMs = 240000, verifyScript = 'check', maxContextLevel = 3 } = {}) {
  ...
  // decide - this is what "РјРёРЅРёРјР°Р»СЊРЅР°СЏ СЃС‚РѕРёРјРѕСЃС‚СЊ РїСЂРё С‚СЂРµР±СѓРµРјРѕРј РєР°С‡РµСЃС‚РІРµ"
  ...
  // Adaptive time budget: once в‰Ґ3 real successful samples exist for this
  ...
  for (const model of orderedModels) {
    ...
    if (isLocal && !usesOllama) continue; // defensive, should never happen given orderedModels вЉ† models
...
const implementGoalCore = implementGoal;
async function implementGoalGuarded(options = {}) {
  const mainRoot = options.mainRoot || process.cwd();
  return sessionGuard.withAgentSessionGuard('agent-adapters', () => implementGoalCore(options), { localHeavy: false });
}

const WORKTREE_SCRATCH_DIR = path.join(sessionGuard.loadPolicy().worktreesRoot, 'agent-invoke');
...
function createIsolatedWorktree(mainRoot, name) {
  ...
  let add = null, branch = null, dir = null;
  for (let attempt = 0; attempt < 4; attempt++) {
    const stamp = `${Date.now()}-${process.pid}-${crypto.randomBytes(4).toString('hex')}`;
    branch = `ai/agent-invoke/${safeName}-${stamp}`;
    dir = path.join(WORKTREE_SCRATCH_DIR, `${safeName}-${stamp}`);
    add = spawnSync('git', ['worktree', 'add', dir, '-b', branch, 'origin/master'], { cwd: mainRoot, encoding: 'utf8', timeout: 60000 });
    if (add.status === 0) break;
    const stderr = String(add.stderr || '');
    const collision = stderr.includes('already exists') || stderr.includes('File exists') || stderr.includes('lock');
    if (collision && attempt < 3) continue;
    break;
  }
  if (!add || add.status !== 0) return { ok: false, retriable: true, error: `git worktree add failed: ${String(add?.stderr || '').slice(-1500)}` };
...
module.exports = {
  ollamaAvailable, queryOllama, opencodeAvailable, implementGoal: implementGoalGuarded, invokeOpencodeOnce, invokeOllamaPatchOnce, assertIsolatedWorktree,
