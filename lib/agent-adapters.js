...
function createIsolatedWorktree(mainRoot, name) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const branchExists = spawnSync('git', ['rev-parse', '--verify', branch], { cwd: mainRoot, encoding: 'utf8' }).status === 0;
    const args = branchExists
      ? ['worktree', 'add', dir, branch]
      : ['worktree', 'add', dir, '-b', branch, 'origin/master'];
    add = spawnSync('git', args, { cwd: mainRoot, encoding: 'utf8', timeout: 60000 });
    if (add.status === 0) break;
    const stderr = String(add.stderr || '');
    if ((stderr.includes('lock') || stderr.includes('File exists') || stderr.includes('already exists')) && attempt < 2) {
      const stop = Date.now() + 150;
      while (Date.now() < stop) { /* wait */ }
      continue;
    }
    break;
  }
