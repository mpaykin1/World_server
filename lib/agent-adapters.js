...
function createIsolatedWorktree(mainRoot, name) {
  ...
  if (fetch1.status !== 0) {
    const revMaster = spawnSync('git', ['rev-parse', '--verify', 'master'], { cwd: mainRoot, encoding: 'utf8' });
    if (revMaster.status === 0) ref = 'master';
    else ref = 'HEAD';
  }
  let add = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    add = spawnSync('git', ['worktree', 'add', dir, '-b', branch, ref], { cwd: mainRoot, encoding: 'utf8', timeout: 60000 });
    if (add.status === 0) break;
    const stderr = String(add.stderr || '');
    if ((stderr.includes('lock') || stderr.includes('File exists')) && attempt < 2) {
