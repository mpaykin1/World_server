...
async function implementGoal({ mainRoot, goal, targetWorktree, models = FREE_OPENCODE_MODELS, timeoutMs = 240000, verifyScript = 'check', maxContextLevel = 3 } = {}) {
  ...
  // Real, evidence-based skip - narrowed after a real false-positive found
  // live this session: the probe is ONE bounded call against ONE model, so
  // it hanging or running slow once is not strong evidence that every
  // model across every context level will also fail (directly observed:
  // in the same benchmark run, a probe-adjacent attempt hung twice before
  // succeeding on a third model, while two other tasks were wrongly
  // skipped entirely by an earlier, overly-broad version of this gate).
  // Only skip the whole chain for states where retrying genuinely cannot
  // help - the CLI/model tier is not reachable at all, or a real 429 was
  // observed. A slow or once-hung probe still lets the real multi-model,
  // multi-level fallback loop below run and find out for itself.
  const health = await agentHealthProbe.probe(mainRoot, { deep: true, useCache: true });
  const HARD_SKIP_STATES = new Set(['RATE_LIMITED', 'NETWORK_DEGRADED', 'MODEL_UNAVAILABLE']);
  if (HARD_SKIP_STATES.has(health.state)) {
    return { ok: false, retriable: true, needsEscalation: false, healthProbe: health, error: `agent health probe reports ${health.state} (${health.reason}) - skipping the free-agent fallback chain, this state means retrying cannot help` };
  }
  ...
  return resourceScheduler.withResourceSlot(mainRoot, 'agent_implement', taskId, async () => {
    ...
    for (const model of orderedModels) {
      for (let level = 1; level <= maxContextLevel; level++) {
        ...
        agentHistory.recordAttempt(mainRoot, {
          ...
          model, provider: 'opencode-free', level, durationMs, success: attempt.ok, classification: attempt.classification,
        ...
        if (attempt.ok) return { ...attempt, attempts, needsEscalation: false, contextLevel: level, scopedFiles: ctx.full ? null : ctx.files, modelRanking: ranking.taskType, healthProbe: health, adaptiveTimeoutMs };
    ...
    return { ok: false, retriable: false, needsEscalation: true, attempts, healthProbe: health, error: `all ${models.length} free-tier model(s) failed across all context levels (last: ${last && last.classification})` };
  }, { maxWaitMs: 120000 });
}
