...
const SCIENCE_DOMAINS = Object.freeze(['visualDestruction','recoveryAnimation','playerDestruction','weapons','npcBehavior','newBuildings','newTextures','roads','destructionPhysics','controls','worldGeneration','multiplayer']);
const DOMAIN_STAGES = Object.freeze(['disabled','planned','experimental','verified-runtime','production-enabled']);
const DOMAIN_GATES = Object.freeze(['implementation','causeEffect','navigator','telemetry','regression','performance','productionEvidence']);
...
function validateAge5Navigator(navigator) {
  ...
  const texts = [navigator.intro, navigator.damage, navigator.regrow, navigator.scienceNote, ...Object.values(navigator.domains || {})].filter(Boolean);
  if (texts.length < 4) throw new Error('Navigator needs intro, damage, regrow and scienceNote.');
...
function domainStageRank(stage) { return DOMAIN_STAGES.indexOf(String(stage || '')); }
function validateDomains(domains) {
  if (!domains || typeof domains !== 'object') throw new Error('Science gameplay domains are required.');
  for (const domain of SCIENCE_DOMAINS) {
    const cfg = domains[domain];
    if (!cfg || !DOMAIN_STAGES.includes(cfg.stage)) throw new Error(`Invalid domain stage: ${domain}.`);
    if (typeof cfg.runtime?.preview !== 'boolean' || typeof cfg.runtime?.production !== 'boolean') throw new Error(`Invalid domain runtime flags: ${domain}.`);
    for (const gate of DOMAIN_GATES) if (typeof cfg.gates?.[gate] !== 'boolean') throw new Error(`Missing domain gate ${domain}.${gate}.`);
    if (['disabled','planned'].includes(cfg.stage) && (cfg.runtime.preview || cfg.runtime.production)) throw new Error(`Inactive domain cannot run: ${domain}.`);
    if (cfg.stage === 'production-enabled') {
      if (!cfg.runtime.production || !DOMAIN_GATES.every(g => cfg.gates[g] === true)) throw new Error(`Production domain is not fully verified: ${domain}.`);
    }
  }
  return true;
}
function domainRuntimeEnabled(contract, domain, target = 'preview') {
  const cfg = contract?.domains?.[domain];
  if (!cfg || !SCIENCE_DOMAINS.includes(domain)) return false;
  if (target === 'production') return cfg.stage === 'production-enabled' && cfg.runtime?.production === true;
  return domainStageRank(cfg.stage) >= domainStageRank('experimental') && cfg.runtime?.preview === true;
}
