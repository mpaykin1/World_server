...
function proposeCycleClosure(contract, context) {
  ...
  for (const candidate of emptyCells) {
    ...
    const distance = context.removed
      ...
      : 0;
    const score = neighborKeys.length * Number(contract.mechanic.neighborWeight || 4)
      - distance * Number(contract.mechanic.distancePenalty || 0.08)
      + deterministicJitter(contract.runId, candidate.x, candidate.y, candidate.z) * 0.01;
