...
function classifyIntent(taskText) {
  ...
  // The coordinator's OpenHuman verifier intentionally reads repository files only.
  // Its inherited master goal often contains incidental git/PR words, which used to
  // win the generic git rule and leave the direct-Ollama call with no model/tools.
  // Preserve that explicit read-only contract before considering incidental terms.
  if (/independent\s+read-only\s+verification/i.test(t) && /\bread\b/i.test(t)) return 'filesystem-read';
