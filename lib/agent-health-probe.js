...
const OLLAMA_TRIVIAL_PROMPT_HEALTHY_MS = 30000; // real observed: 7-15s; generous margin
const OLLAMA_MIN_SAFE_FREE_BYTES_OVER_MODEL = 1.5; // free RAM must be >= 1.5x the model's on-disk size
...
function classifyOllama({ available, freeBytes, modelSizeBytes, wasLoaded, tier2 }) {
  ...
  if (modelSizeBytes != null && freeBytes != null && freeBytes < modelSizeBytes * OLLAMA_MIN_SAFE_FREE_BYTES_OVER_MODEL) {
    return { state: 'OLLAMA_OOM', reason: `only ${(freeBytes / 1e9).toFixed(2)}GB free RAM, below the ${OLLAMA_MIN_SAFE_FREE_BYTES_OVER_MODEL}x-model-size safety margin for a ${(modelSizeBytes / 1e9).toFixed(2)}GB model — refusing to attempt a load that could thrash or OOM this machine` };
  }
