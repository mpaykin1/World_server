...
async function tier2(timeoutMs = 20000) {
  const goalFile = path.join(sessionGuard.ensureRoots().scratchRoot, `health-probe-${Date.now()}.txt`);
  fs.writeFileSync(goalFile, 'Reply with exactly one word: HEALTHY');
  const start = Date.now();
  const r = await agentAdapters.runWithTreeKill('opencode', ['run', '--model', agentAdapters.FREE_OPENCODE_MODELS[0], '--dir', sessionGuard.ensureRoots().scratchRoot, '--format', 'json', 'Follow the instructions in the attached file exactly.', '-f', goalFile], { timeout: timeoutMs });
...
function classifyOllama({ available, freeBytes, modelSizeBytes, wasLoaded, tier2 }) {
  ...
  if (freeBytes != null) {
    if (wasLoaded) {
      ...
      return { state: 'OLLAMA_OOM', reason: `only ${(freeBytes / 1e9).toFixed(2)}GB free RAM, below the ${OLLAMA_MIN_SAFE_FREE_BYTES_OVER_MODEL}x-model-size safety margin for a COLD load of a ${(modelSizeBytes / 1e9).toFixed(2)}GB model вЂ” refusing to attempt a load that could thrash or OOM this machine` };
    }
  }
  ...
  if (!tier2.ok || !tier2.gotAnyOutput) {
    return { state: wasLoaded ? 'OLLAMA_SLOW' : 'OLLAMA_MODEL_LOADING', reason: wasLoaded ? `model was already loaded but the trivial probe still produced no output within ${tier2.durationMs}ms` : `model was not yet loaded вЂ” first call includes real disk-load time, probe did not complete within ${tier2.durationMs}ms` };
  }
