...
function resampleNearest(image, width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      pixels[dst] = image.pixels[src]; pixels[dst + 1] = image.pixels[src + 1]; pixels[dst + 2] = image.pixels[src + 2]; pixels[dst + 3] = image.pixels[src + 3];
    }
  }
  return { width, height, pixels };
}
...
function compareReferenceRuntime(referenceInput, runtimeInput, options = {}) {
  ...
  return {
    ...
    comparisonWidth, comparisonHeight, resampled,
